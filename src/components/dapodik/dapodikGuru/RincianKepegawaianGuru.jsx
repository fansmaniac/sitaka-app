...
// =====================================================================
// MAIN COMPONENT: RINCIAN KEPEGAWAIAN GURU
// =====================================================================
export default function RincianKepegawaianGuru({ 
  ...
  const dataKecamatan = useMemo(() => {
    ...
    baseData.forEach(item => {
      ...
      if (!mapAgg.has(keyId)) {
        mapAgg.set(keyId, { namaWilayah: keyId, peg_pns: 0, peg_pppk: 0, peg_gty: 0, peg_honor: 0, peg_lain: 0, total: 0 });
      }
  ...
  const downloadExcelRincian = async () => {
    ...
    if (activeModalTab === 'KECAMATAN') {
      const worksheet = workbook.addWorksheet('Rekap Kepegawaian Wilayah');
      worksheet.columns = [
      ...
      const totalRow = worksheet.addRow({ namaWilayah: 'TOTAL', peg_pns: totalsKecamatan.peg_pns, peg_pppk: totalsKecamatan.peg_pppk, peg_gty: totalsKecamatan.peg_gty, peg_honor: totalsKecamatan.peg_honor, peg_lain: totalsKecamatan.peg_lain, total: totalsKecamatan.total });
      ...
      link.download = `Rincian_Guru_Kepegawaian_${initialWilayah}_${safeName}.xlsx`;
      link.click();
    } else {
      const worksheet = workbook.addWorksheet('Daftar Sekolah & Kepegawaian');
      worksheet.columns = [
      ...
      const link = document.createElement('a');
      link.href = URL.createObjectURL(blob);
      link.download = `Daftar_Guru_Sekolah_Kepegawaian_${initialWilayah}_${safeName}.xlsx`;
      link.click();
    }
  };
        ...
        {activeModalTab === 'SEKOLAH' && (
          <div className="bg-white px-6 pt-4 pb-0 flex gap-2 overflow-x-auto scrollbar-hide shrink-0 z-10 relative">
            <button 
              ...
              className={`px-5 py-2 rounded-xl font-black uppercase text-[10px] md:text-xs transition-all whitespace-nowrap border ${activeJenjangTab === mappedJenjang ? 'bg-teal-600 text-white border-teal-600 shadow-md' : 'bg-gray-50 text-gray-500 border-gray-200 hover:bg-gray-100'}`}
            >
