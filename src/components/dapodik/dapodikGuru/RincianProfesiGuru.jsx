...
// =====================================================================
// MAIN COMPONENT: RINCIAN SERTIFIKASI PROFESI GURU
// =====================================================================
export default function RincianProfesiGuru({ 
  isOpen, 
  onClose, 
  data = [], 
  ...
  const dataKecamatan = useMemo(() => {
    ...
    baseData.forEach(item => {
      ...
      if (!mapAgg.has(keyId)) {
        mapAgg.set(keyId, { namaWilayah: keyId, sert_sudah: 0, sert_belum: 0, total: 0 });
      }
  ...
  const totalsKecamatan = useMemo(() => {
    return dataKecamatan.reduce((acc, curr) => {
      acc.sert_sudah += curr.sert_sudah;
      acc.sert_belum += curr.sert_belum;
      acc.total += curr.total;
      return acc;
    }, { sert_sudah: 0, sert_belum: 0, total: 0 });
  }, [dataKecamatan]);
  ...
  const downloadExcelRincian = async () => {
    ...
    if (activeModalTab === 'KECAMATAN') {
      const worksheet = workbook.addWorksheet('Rekap Sertifikasi Wilayah');
      worksheet.columns = [
        { header: isModeSemua ? 'Kabupaten/Kota' : 'Kecamatan', key: 'namaWilayah', width: 30 },
        { header: 'Sudah Sertifikasi', key: 'sert_sudah', width: 20 },
        { header: 'Belum Sertifikasi', key: 'sert_belum', width: 20 },
        { header: 'Total Guru', key: 'total', width: 18 },
      ];
      dataKecamatan.forEach(item => worksheet.addRow(item));
      const totalRow = worksheet.addRow({ namaWilayah: 'TOTAL', sert_sudah: totalsKecamatan.sert_sudah, sert_belum: totalsKecamatan.sert_belum, total: totalsKecamatan.total });
      ...
      link.download = `Rincian_Guru_Sertifikasi_${initialWilayah}_${safeName}.xlsx`;
      link.click();
    } else {
      const worksheet = workbook.addWorksheet('Daftar Sekolah & Sertifikasi');
      worksheet.columns = [
        ...
        { header: 'Sudah Sertifikasi', key: 'sert_sudah', width: 18 },
        { header: 'Belum Sertifikasi', key: 'sert_belum', width: 18 },
        { header: 'Total Guru', key: 'total', width: 18 },
      ];
      ...
      const link = document.createElement('a');
      link.href = URL.createObjectURL(blob);
      link.download = `Daftar_Guru_Sekolah_Sertifikasi_${initialWilayah}_${safeName}.xlsx`;
      link.click();
    }
  };
  ...
  return createPortal(
    <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-in fade-in duration-200" onClick={onClose}>
      <div className="bg-white w-full max-w-7xl h-[90vh] rounded-3xl shadow-2xl flex flex-col overflow-hidden animate-in zoom-in-95 duration-300" onClick={e => e.stopPropagation()}>
        ...
        {/* HEADER MODAL */}
        <div className="bg-amber-600 px-6 py-5 flex items-center justify-between shrink-0">
          <div className="flex items-center gap-3 text-white">
            <div className="bg-white/20 p-2 rounded-xl"><Award size={24} /></div>
            <div>
              <h2 className="text-xl font-black uppercase tracking-tighter leading-none">
                Rincian Sertifikasi Profesi Guru
              </h2>
              <p className="text-amber-200 text-sm font-bold uppercase tracking-widest mt-1 flex gap-2">
          ...
          <div className="flex items-center gap-3">
            ...
            <button onClick={onClose} className="p-2 bg-amber-800 hover:bg-red-500 text-white rounded-xl transition-colors">
              <X size={24} />
            </button>
          </div>
        </div>
        ...
        <div className="bg-amber-50 px-6 pt-3 flex gap-2 border-b border-amber-100 shrink-0">
          <button 
            onClick={() => setActiveModalTab('KECAMATAN')}
            className={`px-6 py-2.5 rounded-t-xl font-black uppercase text-xs transition-all border-b-4 ${activeModalTab === 'KECAMATAN' ? 'bg-white text-amber-700 border-amber-700 shadow-[0_-4px_10px_rgba(0,0,0,0.05)]' : 'bg-transparent text-amber-500 border-transparent hover:text-amber-700 hover:bg-amber-100/50'}`}
          >
            <div className="flex items-center gap-2"><MapPin size={16}/> Per Kecamatan</div>
          </button>
          <button 
            onClick={() => setActiveModalTab('SEKOLAH')}
            className={`px-6 py-2.5 rounded-t-xl font-black uppercase text-xs transition-all border-b-4 ${activeModalTab === 'SEKOLAH' ? 'bg-white text-amber-700 border-amber-700 shadow-[0_-4px_10px_rgba(0,0,0,0.05)]' : 'bg-transparent text-amber-500 border-transparent hover:text-amber-700 hover:bg-amber-100/50'}`}
          >
        ...
        {activeModalTab === 'SEKOLAH' && (
          <div className="bg-white px-6 pt-4 pb-0 flex gap-2 overflow-x-auto scrollbar-hide shrink-0 z-10 relative">
            <button 
              ...
              className={`px-5 py-2 rounded-xl font-black uppercase text-[10px] md:text-xs transition-all whitespace-nowrap border ${activeJenjangTab === mappedJenjang ? 'bg-amber-600 text-white border-amber-600 shadow-md' : 'bg-gray-50 text-gray-500 border-gray-200 hover:bg-gray-100'}`}
            >
            ...
            {availableTabs.map(j => (
              <button 
                ...
                className={`px-5 py-2 rounded-xl font-black uppercase text-[10px] md:text-xs transition-all whitespace-nowrap border ${activeJenjangTab === j ? 'bg-amber-600 text-white border-amber-600 shadow-md' : 'bg-gray-50 text-gray-500 border-gray-200 hover:bg-gray-100'}`}
              >
                {j}
              </button>
            ))}
        ...
        <div className={`bg-white px-6 py-4 border-b border-gray-200 flex flex-wrap gap-4 items-center shrink-0 shadow-sm z-10 ${activeModalTab === 'SEKOLAH' ? 'border-t-0 pt-3' : ''}`}>
          <div className="relative flex-1 min-w-[250px]">
            ...
            <input 
              ...
              className="w-full pl-10 pr-10 py-2.5 rounded-xl border border-gray-200 outline-none focus:border-amber-500 focus:ring-2 focus:ring-amber-200 font-bold text-gray-700"
            />
          </div>
              ...
              {dataKecamatan.length > 0 && (
                <tfoot className="sticky bottom-0 z-10 shadow-[0_-4px_10px_rgba(0,0,0,0.04)]">
                  <tr className="bg-amber-100 text-center font-black uppercase text-xs border-t-2 border-amber-200">
                    ...
                    <td className="px-3 py-4 text-emerald-800 text-base border-y border-amber-200">{totalsKecamatan.sert_sudah.toLocaleString()}</td>
                    <td className="px-3 py-4 text-red-800 text-base border-y border-amber-200">{totalsKecamatan.sert_belum.toLocaleString()}</td>
                    <td className="px-4 py-4 text-amber-950 text-lg border-y border-r border-amber-200 rounded-r-2xl bg-amber-200/50">
            ...
            <table className="w-full text-center border-separate border-spacing-y-2">
              <thead className="sticky top-0 bg-white z-10 shadow-sm rounded-xl">
                <tr className="text-[10px] font-black uppercase text-gray-500">
                  ...
                  <th className="px-3 py-3 text-amber-600">Jenjang</th>
                  <th className="px-3 py-3 text-orange-600">Status</th>
                  <th className="px-3 py-3 text-emerald-600">Sertifikasi</th>
                  <th className="px-3 py-3 text-red-600">Belum</th>
              ...
              <tbody>
                {currentRows.map((row, idx) => (
                  <tr key={idx} className="bg-white shadow-sm hover:shadow-md hover:scale-[1.01] transition-all group">
                    ...
                    <td className="px-3 py-3 font-bold text-amber-600 text-xs border-y border-gray-100 uppercase">{row.jenjang}</td>
                    <td className="px-3 py-3 font-bold text-orange-600 text-xs border-y border-gray-100 uppercase">{row.status}</td>
                    
                    <td className="px-3 py-3 font-bold text-emerald-600 text-sm border-y border-gray-100 bg-emerald-50/20">{row.sert_sudah.toLocaleString()}</td>
                    <td className="px-3 py-3 font-bold text-red-600 text-sm border-y border-gray-100 bg-red-50/20">{row.sert_belum.toLocaleString()}</td>

                    <td className="px-4 py-3 font-black text-amber-800 text-lg border-y border-r border-gray-100 bg-amber-50/50 rounded-r-2xl">
              ...
              {dataSekolah.length > 0 && (
                <tfoot className="sticky bottom-0 z-10 shadow-[0_-4px_10px_rgba(0,0,0,0.04)]">
                  <tr className="bg-amber-100 text-center font-black uppercase text-xs border-t-2 border-amber-200">
                    ...
                    <td className="px-3 py-4 text-red-800 text-base border-y border-amber-200">{totalGuruSekolah.sert_belum.toLocaleString()}</td>
                    <td className="px-4 py-4 text-amber-950 text-lg border-y border-r border-amber-200 rounded-r-2xl bg-amber-200/50">
        ...
        <div className="bg-white p-4 border-t border-gray-200 flex items-center justify-between shrink-0 rounded-b-3xl">
          <div className="flex flex-col">
            <p className="text-xs font-bold text-gray-500">
              Menampilkan <span className="text-gray-800">{activeData.length === 0 ? 0 : startIndex + 1}</span> - <span className="text-gray-800">{Math.min(startIndex + rowsPerPage, activeData.length)}</span> dari <span className="text-amber-700 font-black">{activeData.length}</span> baris
            </p>
            {displayLastUpdated && (
          ...
          <div className="flex items-center gap-2">
            <button disabled={currentPage === 1} onClick={() => goToPage(currentPage - 1)} className="p-2 rounded-xl bg-gray-50 border border-gray-200 hover:bg-amber-50 disabled:opacity-50 transition-colors"><ChevronLeft size={16} /></button>
            <span className="text-xs font-black text-gray-600 px-2">Hal {currentPage} / {totalPages}</span>
            <button disabled={currentPage === totalPages || totalPages === 0} onClick={() => goToPage(currentPage + 1)} className="p-2 rounded-xl bg-gray-50 border border-gray-200 hover:bg-amber-50 disabled:opacity-50 transition-colors"><ChevronRight size={16} /></button>
          </div>
        </div>
