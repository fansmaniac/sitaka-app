...
export default function RincianPDJenjangNonFormal({ 
  ...
  const dataKecamatan = useMemo(() => {
    ...
    baseData.forEach(item => {
      ...
      const pd_l = parseInt(getVal(item, 'pd_l')) || parseInt(getVal(item, 'l')) || 0;
      const pd_p = parseInt(getVal(item, 'pd_p')) || parseInt(getVal(item, 'p')) || 0;
  ...
  const dataSekolah = useMemo(() => {
    ...
    return validData.map(item => {
      const nf_l = parseInt(getVal(item, 'pd_l')) || parseInt(getVal(item, 'l')) || 0;
      const nf_p = parseInt(getVal(item, 'pd_p')) || parseInt(getVal(item, 'p')) || 0;
      const pd_total = parseInt(getVal(item, 'pd_total')) || (nf_l + nf_p);
  ...
  const downloadExcelRincian = async () => {
    ...
    if (activeModalTab === 'KECAMATAN') {
      ...
      const totalRow = worksheet.addRow({
        ...
        nf_l: totalsKecamatan.nf_l,
        nf_p: totalsKecamatan.nf_p,
      ...
      link.download = `Rincian_PD_NonFormal_Kecamatan_${initialWilayah}.xlsx`;
      link.click();
    } else {
      // Tab SEKOLAH
      ...
      worksheet.columns = [
        ...
        { header: 'PD Laki-Laki', key: 'nf_l', width: 18 },
        { header: 'PD Perempuan', key: 'nf_p', width: 18 },
        { header: 'Total Peserta Didik', key: 'total', width: 22 },
      ];
      ...
      const link = document.createElement('a');
      link.href = URL.createObjectURL(blob);
      link.download = `Daftar_Sekolah_PD_NonFormal_${initialWilayah}.xlsx`;
      link.click();
    }
  };
              ...
              {dataKecamatan.length > 0 && (
                <tfoot className="sticky bottom-0 z-10 shadow-[0_-4px_10px_rgba(0,0,0,0.04)]">
                  <tr className="bg-teal-100 text-center font-black uppercase text-xs border-t-2 border-teal-200">
                    ...
                    <td className="px-4 py-4 text-sky-800 text-base border-y border-teal-200">{totalsKecamatan.nf_l.toLocaleString()}</td>
                    <td className="px-4 py-4 text-pink-800 text-base border-y border-teal-200">{totalsKecamatan.nf_p.toLocaleString()}</td>
                ...
                <tfoot className="sticky bottom-0 z-10 shadow-[0_-4px_10px_rgba(0,0,0,0.04)]">
                  <tr className="bg-teal-100 text-center font-black uppercase text-xs border-t-2 border-teal-200">
                    <td colSpan="6" className="px-4 py-4 text-left rounded-l-2xl border-y border-l border-teal-200 text-teal-900">
                      TOTAL DARI {dataSekolah.length} LEMBAGA
                    </td>
                    <td className="px-3 py-4 text-sky-800 text-base border-y border-teal-200">{totalsSekolah.nf_l.toLocaleString()}</td>
                    <td className="px-3 py-4 text-pink-800 text-base border-y border-teal-200">{totalsSekolah.nf_p.toLocaleString()}</td>
                    <td className="px-4 py-4 text-teal-950 text-lg border-y border-r border-teal-200 rounded-r-2xl bg-teal-200/50">
