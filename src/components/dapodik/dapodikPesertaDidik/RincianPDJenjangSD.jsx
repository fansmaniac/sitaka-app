...
const JENJANG_GROUPS = {
  'SD': ['SD', 'SPK SD'] // Sesuai dengan filter SD utama
};

const identifyJenjangGroup = (jenjangDb) => {
  const j = String(jenjangDb).trim().toUpperCase();
  if (JENJANG_GROUPS['SD'].includes(j)) return 'SD';
  return null;
};
...
// MAIN COMPONENT
// =====================================================================
export default function RincianPDJenjangSD({ 
  isOpen, 
  onClose, 
  data = [], 
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      setSearchTerm('');
      setFilterWilayah('SEMUA'); // Kunci perbaikan
  ...
  const dataKecamatan = useMemo(() => {
    ...
    baseData.forEach(item => {
      ...
      // Ambil properti yang sudah bersih dari Tahap 3
      ...
      row.sd_1 += t1; row.sd_2 += t2; row.sd_3 += t3;
      row.sd_4 += t4; row.sd_5 += t5; row.sd_6 += t6;
