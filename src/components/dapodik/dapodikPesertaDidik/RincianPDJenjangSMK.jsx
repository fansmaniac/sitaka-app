...
const identifyJenjangGroup = (jenjangDb) => {
  ...
  if (JENJANG_GROUPS['SMK'].includes(j)) return 'SMK';
  return null;
};
...
// MAIN COMPONENT
// =====================================================================
export default function RincianPDJenjangSMK({ 
  isOpen, 
  onClose, 
  data = [], 
  ...
  // PERBAIKAN BUG: Selalu inisialisasi dengan 'SEMUA' agar daftar kecamatan tidak kosong
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      setSearchTerm('');
      setFilterWilayah('SEMUA'); // Kunci perbaikan bug
  ...
  const totalsKecamatan = useMemo(() => {
    return dataKecamatan.reduce((acc, curr) => {
      acc.smk_10 += curr.smk_10; 
      acc.smk_11 += curr.smk_11; 
      acc.smk_12 += curr.smk_12;
      acc.total += curr.total;
      return acc;
    }, { smk_10: 0, smk_11: 0, smk_12: 0, total: 0 });
  }, [dataKecamatan]);
  ...
  const downloadExcelRincian = async () => {
    ...
    if (activeModalTab === 'KECAMATAN') {
      ...
      worksheet.columns = [
        ...
        { header: 'Kelas 10', key: 'smk_10', width: 15 },
        { header: 'Kelas 11', key: 'smk_11', width: 15 },
        { header: 'Kelas 12', key: 'smk_12', width: 15 },
      ...
      const totalRow = worksheet.addRow({
        ...
        smk_10: totalsKecamatan.smk_10, 
        smk_11: totalsKecamatan.smk_11, 
        smk_12: totalsKecamatan.smk_12,
        total: totalsKecamatan.total
      });

      // Styling Header (Warna Ungu SMK)
      ...
      link.download = `Rincian_PD_SMK_Kecamatan_${initialWilayah}.xlsx`;
      link.click();
    } else {
      // Tab SEKOLAH
      ...
      worksheet.columns = [
        ...
        { header: 'Kelas 10', key: 'smk_10', width: 12 },
        { header: 'Kelas 11', key: 'smk_11', width: 12 },
        { header: 'Kelas 12', key: 'smk_12', width: 12 },
        { header: 'Total Peserta Didik', key: 'total', width: 22 },
      ];
      ...
      const link = document.createElement('a');
      link.href = URL.createObjectURL(blob);
      link.download = `Daftar_Sekolah_PD_SMK_${initialWilayah}.xlsx`;
      link.click();
    }
  };
  ...
  return createPortal(
    <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-in fade-in duration-200" onClick={onClose}>
      <div className="bg-white w-full max-w-7xl h-[90vh] rounded-3xl shadow-2xl flex flex-col overflow-hidden animate-in zoom-in-95 duration-300" onClick={e => e.stopPropagation()}>
        ...
        {/* HEADER MODAL */}
        <div className="bg-purple-600 px-6 py-5 flex items-center justify-between shrink-0">
          <div className="flex items-center gap-3 text-white">
            <div className="bg-white/20 p-2 rounded-xl"><GraduationCap size={24} /></div>
            <div>
              <h2 className="text-xl font-black uppercase tracking-tighter leading-none">
                Rincian Peserta Didik SMK
              </h2>
        ...
        <div className="flex-1 overflow-auto bg-gray-50/50 p-4 custom-scrollbar">
            ...
            <table className="w-full text-center border-separate border-spacing-y-2">
              <thead className="sticky top-0 bg-white z-10 shadow-sm rounded-xl">
                <tr className="text-[10px] font-black uppercase text-gray-500">
                  ...
                  <th className="px-4 py-3 text-rose-500">Kelas 10</th>
                  <th className="px-4 py-3 text-pink-500">Kelas 11</th>
                  <th className="px-4 py-3 text-purple-500">Kelas 12</th>
              ...
              <tbody>
                {currentRows.map((row, idx) => (
                  <tr key={idx} className="bg-white shadow-sm hover:shadow-md hover:scale-[1.01] transition-all group">
                    ...
                    <td className="px-4 py-4 font-bold text-rose-500 text-sm border-y border-gray-100 bg-purple-50/20">{row.smk_10.toLocaleString()}</td>
                    <td className="px-4 py-4 font-bold text-pink-500 text-sm border-y border-gray-100 bg-purple-50/20">{row.smk_11.toLocaleString()}</td>
                    <td className="px-4 py-4 font-bold text-purple-500 text-sm border-y border-gray-100 bg-purple-50/20">{row.smk_12.toLocaleString()}</td>
              ...
              {dataKecamatan.length > 0 && (
                <tfoot className="sticky bottom-0 z-10 shadow-[0_-4px_10px_rgba(0,0,0,0.04)]">
                  <tr className="bg-purple-100 text-center font-black uppercase text-xs border-t-2 border-purple-200">
                    ...
                    <td className="px-4 py-4 text-rose-600 border-y border-purple-200 text-base">{totalsKecamatan.smk_10.toLocaleString()}</td>
                    <td className="px-4 py-4 text-pink-600 border-y border-purple-200 text-base">{totalsKecamatan.smk_11.toLocaleString()}</td>
                    <td className="px-4 py-4 text-purple-600 border-y border-purple-200 text-base">{totalsKecamatan.smk_12.toLocaleString()}</td>
            ...
            <table className="w-full text-center border-separate border-spacing-y-2">
              <thead className="sticky top-0 bg-white z-10 shadow-sm rounded-xl">
                <tr className="text-[10px] font-black uppercase text-gray-500">
                  ...
                  <th className="px-4 py-3 text-rose-500">Kls 10</th>
                  <th className="px-4 py-3 text-pink-500">Kls 11</th>
                  <th className="px-4 py-3 text-purple-500">Kls 12</th>
                  <th className="px-4 py-3 rounded-r-xl text-gray-800">Total PD</th>
                </tr>
              </thead>
              <tbody>
                {currentRows.map((row, idx) => (
                  <tr key={idx} className="bg-white shadow-sm hover:shadow-md hover:scale-[1.01] transition-all group">
                    ...
                    <td className="px-4 py-3 font-bold text-rose-500 text-sm border-y border-gray-100 bg-purple-50/20">{row.smk_10.toLocaleString()}</td>
                    <td className="px-4 py-3 font-bold text-pink-500 text-sm border-y border-gray-100 bg-purple-50/20">{row.smk_11.toLocaleString()}</td>
                    <td className="px-4 py-3 font-bold text-purple-500 text-sm border-y border-gray-100 bg-purple-50/20">{row.smk_12.toLocaleString()}</td>
              ...
              {dataSekolah.length > 0 && (
                <tfoot className="sticky bottom-0 z-10 shadow-[0_-4px_10px_rgba(0,0,0,0.04)]">
                  <tr className="bg-purple-100 text-center font-black uppercase text-xs border-t-2 border-purple-200">
                    <td colSpan="5" className="px-4 py-4 text-left rounded-l-2xl border-y border-l border-purple-200 text-purple-900">
                      TOTAL DARI {dataSekolah.length} SEKOLAH
                    </td>
                    <td className="px-4 py-4 text-rose-600 border-y border-purple-200 text-base">{totalsSekolah.smk_10.toLocaleString()}</td>
                    <td className="px-4 py-4 text-pink-600 border-y border-purple-200 text-base">{totalsSekolah.smk_11.toLocaleString()}</td>
                    <td className="px-4 py-4 text-purple-600 border-y border-purple-200 text-base">{totalsSekolah.smk_12.toLocaleString()}</td>
                    <td className="px-4 py-4 text-purple-950 text-lg border-y border-r border-purple-200 rounded-r-2xl bg-purple-200/50">
          ...
          {activeData.length === 0 && (
             <div className="py-20 flex flex-col items-center opacity-30 text-gray-500">
               ...
               <p className="font-black uppercase tracking-widest text-xl">Tidak Ada Data SMK</p>
             </div>
          )}
        </div>
