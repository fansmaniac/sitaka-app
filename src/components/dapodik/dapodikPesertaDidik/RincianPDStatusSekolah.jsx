...
// PENGELOMPOKAN SINKRON 100% DENGAN DapodikPesertaDidik.jsx
...
// MAIN COMPONENT
// =====================================================================
export default function RincianPDStatusSekolah({ 
  isOpen, 
  onClose, 
  data = [], 
  ...
  // PERBAIKAN: Harus 'SEMUA' agar dropdown filter kecamatan selalu utuh
  ...
  }, [isOpen, initialWilayah, activeJenjang]);

  // Reset pagination saat pencarian atau filter berubah
  useEffect(() => { 
    setCurrentPage(1); 
  }, [searchTerm, filterWilayah, filterWilayahSekolah, activeModalTab]);
  ...
  // Ekstrak Daftar Wilayah (Kabupaten/Kecamatan)
  ...
  const dataKecamatan = useMemo(() => {
    ...
    baseData.forEach(item => {
      ...
      const pd_total = item.pd_total || 0;

      if (isNegeri) row.negeri += pd_total;
      else row.swasta += pd_total;
  ...
  }, [data, isModeSemua, initialWilayah, filterWilayah, searchTerm, activeJenjang]);
  ...
  const dataSekolah = useMemo(() => {
    ...
    data.forEach(item => {
      ...
      const jenjangDb = String(item.bentuk_pendidikan || '').trim().toUpperCase();
      if (activeJenjang !== 'SEMUA' && activeJenjang !== 'SEMUA JENJANG') {
        if (!validJenjangList.includes(jenjangDb)) return;
      }
      ...
      const namaSekolah = String(item.nama || '-').toUpperCase();
      ...
      row.total += item.pd_total || 0;
    });

    return Array.from(mapSekolah.values()).sort((a, b) => b.total - a.total);
  }, [data, isModeSemua, initialWilayah, activeJenjang, filterWilayahSekolah, searchTerm]);

  const totalSiswaSekolah = useMemo(() => {
  ...
  const downloadExcelRincian = async () => {
    ...
    if (activeModalTab === 'KECAMATAN') {
      ...
      worksheet.columns = [
        ...
        { header: 'PD Negeri', key: 'negeri', width: 18 },
        { header: 'PD Swasta', key: 'swasta', width: 18 },
      ...
      link.download = `Rincian_PD_Status_${isModeSemua ? 'Provinsi' : filterWilayah}_${safeJenjang}.xlsx`;
      link.click();
    } else {
      const worksheet = workbook.addWorksheet('Daftar Sekolah & PD');
      worksheet.columns = [
      ...
      const link = document.createElement('a');
      link.href = URL.createObjectURL(blob);
      link.download = `Daftar_PD_Sekolah_${initialWilayah}_${safeJenjang}.xlsx`;
      link.click();
    }
  };
          ...
          <div className="flex items-center gap-3 text-white">
            ...
            <div>
              <h2 className="text-xl font-black uppercase tracking-tighter leading-none">
                Rincian Status Sekolah Peserta Didik
              </h2>
            ...
            <table className="w-full text-center border-separate border-spacing-y-2">
              <thead className="sticky top-0 bg-white z-10 shadow-sm rounded-xl">
                <tr className="text-[10px] font-black uppercase text-gray-500">
                  ...
                  <th className="px-4 py-3 text-blue-600">PD Negeri</th>
                  <th className="px-4 py-3 text-orange-600">PD Swasta</th>
          ...
          {activeModalTab === 'SEKOLAH' && (
            <table className="w-full text-center border-separate border-spacing-y-2">
              ...
              {dataSekolah.length > 0 && (
                <tfoot className="sticky bottom-0 z-10 shadow-[0_-4px_10px_rgba(0,0,0,0.04)]">
                  <tr className="bg-blue-100 text-center font-black uppercase text-xs border-t-2 border-blue-200">
                    <td colSpan="6" className="px-4 py-4 text-left rounded-l-2xl border-y border-l border-blue-200 text-blue-900">
                      TOTAL PESERTA DIDIK DARI {dataSekolah.length} SEKOLAH
                    </td>
                    <td className="px-4 py-4 text-blue-950 text-lg border-y border-r border-blue-200 rounded-r-2xl bg-blue-200/50">
                      {totalSiswaSekolah.toLocaleString()}
                    </td>
                  </tr>
                </tfoot>
              )}
