...
import { MapPin, Info, Search, Download, Loader2, Activity, Layers, GraduationCap, Users, School } from 'lucide-react';
...
const initDB = (dbName, storeName) => {
  return new Promise((resolve, reject) => {
    const request = indexedDB.open(dbName, 2); // Pakai versi 2 agar data fresh
...
export default function RasioRombelVsPD({ selectedYear }) {
  ...
  // --- FETCH DATA (Membaca Chunks Sekolah & Siswa, Lalu Di-Merge) ---
  useEffect(() => {
    const fetchData = async () => {
      setLoading(true);
      setError(null);
      const cacheKeySekolah = `sekolah_agregasi_v1_${selectedYear}`;
      const cacheKeySiswa = `siswa_agregasi_v2_${selectedYear}`;
      ...
      try {
        ...
        let siswaData = await getFromCache("SitakaCacheDB_SiswaModul_Agregasi", "siswaDataAgg", cacheKeySiswa);
        let lastUpdSiswa = '';
        ...
        if (!siswaData || !Array.isArray(siswaData) || siswaData.length === 0) {
            ...
            if (summarySnap.exists()) {
                ...
                if(siswaData.length > 0) await saveToCache("SitakaCacheDB_SiswaModul_Agregasi", "siswaDataAgg", cacheKeySiswa, siswaData);
            }
        }

        // FALLBACK PENGECEKAN
        if (!sekolahData || sekolahData.length === 0 || !siswaData || siswaData.length === 0) {
            setError(`Data Sekolah (Tahap 1) atau Siswa (Tahap 3) tahun ${selectedYear} belum dikalkulasi.`);
            setMergedData([]);
        } else {
            // --- 3. MERGE DATA SEKOLAH & SISWA (REALTIME BY NPSN) ---
            const mapSekolah = new Map();
            sekolahData.forEach(s => {
                if(s && s.npsn) mapSekolah.set(s.npsn, { ...s, pd_aktual: 0 });
            });
            siswaData.forEach(p => {
                if(p && p.npsn && mapSekolah.has(p.npsn)){
            ...
            const useDate = lastUpdSiswa || lastUpdSekolah;
            if(useDate){
  ...
  const processedData = useMemo(() => {
    ...
    mergedData.forEach(item => {
       ...
       if (isModeSemua || kabDb === filterWilayah) {
           ...
           if (t1Row) {
               if (isNegeri) { t1Row.rombel_n += rombelTotal; t1Row.pd_n += pdTotal; }
               else { t1Row.rombel_s += rombelTotal; t1Row.pd_s += pdTotal; }
    ...
    <div className="w-full flex flex-col gap-8 animate-in slide-in-from-bottom-8 duration-500">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-white p-6 rounded-[2rem] shadow-sm border border-gray-100">
        <div>
          ...
          <p className="text-xs font-bold text-gray-400 uppercase tracking-widest mt-1">Modul Analisa Kepadatan & Kapasitas Kelas</p>
        </div>
      ...
      <div className="bg-orange-50 border border-orange-200 p-6 rounded-3xl flex flex-col md:flex-row items-start gap-6 shadow-sm mb-12">
        ...
        <div className="text-sm text-orange-900 leading-relaxed w-full">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-x-8 gap-y-3 font-bold opacity-90">
            <div className="flex items-center gap-2"><div className="w-2 h-2 rounded-full bg-orange-500"></div> TK, KB, SPS, TPA: Max 15 PD / Rombel</div>
