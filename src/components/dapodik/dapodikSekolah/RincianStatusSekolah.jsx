...
// =====================================================================
// MAPPING STRUKTUR JENJANG
// =====================================================================
const KATEGORI_MAPPING = {
  'PAUD': ['TK', 'KB', 'TPA', 'SPS', 'PAUD'], 
  'SD': ['SD', 'SPK SD'],
  'SMP': ['SMP', 'SPK SMP'],
  'SMA': ['SMA', 'SPK SMA'],
  'SMK': ['SMK'],
...
export default function RincianStatusSekolah({ 
  ...
  }, [isOpen, initialWilayah, defaultJenjangTab]);
  ...
  const dataKecamatan = useMemo(() => {
    ...
    // Untuk rekap kecamatan, kita harus pastikan menggabungkan semua sub-jenjang (Misal SD + SPK SD)
  ...
  const dataSekolah = useMemo(() => {
    ...
    let validData = data.filter(item => {
      ...
      if (filterWilayahSekolah !== 'SEMUA') {
        let keyId = isModeSemua ? kabDb : String(item.kecamatan || 'TIDAK DIKETAHUI').trim().toUpperCase();
    ...
    // Karena ini menghitung Unit Sekolah, kita gunakan data array secara langsung
  ...
  }, [data, isModeSemua, initialWilayah, activeJenjangTab, filterStatus, filterWilayahSekolah, searchTerm]);
  ...
  const downloadExcelRincian = async () => {
    ...
    if (activeModalTab === 'KECAMATAN') {
      ...
      worksheet.columns = [
        ...
        { header: 'Total Sekolah', key: 'total', width: 22 },
      ];
      ...
      link.download = `Rincian_Status_Kecamatan_${initialWilayah}_${safeJenjang}.xlsx`;
      link.click();
    } else {
      ...
      const link = document.createElement('a');
      link.href = URL.createObjectURL(blob);
      link.download = `Daftar_Sekolah_${initialWilayah}_${safeActiveTab}.xlsx`;
      link.click();
    }
  };
        ...
        {activeModalTab === 'SEKOLAH' && (
          <div className="bg-white px-6 pt-4 pb-0 flex gap-2 overflow-x-auto scrollbar-hide shrink-0 z-10 relative">
            <button 
              ...
              className={`px-5 py-2 rounded-xl font-black uppercase text-[10px] md:text-xs transition-all whitespace-nowrap border ${activeJenjangTab === defaultJenjangTab ? 'bg-blue-600 text-white border-blue-600 shadow-md' : 'bg-gray-50 text-gray-500 border-gray-200 hover:bg-gray-100'}`}
            >
          ...
          {activeModalTab === 'SEKOLAH' && (
            <table className="w-full text-center border-separate border-spacing-y-2">
              ...
              {dataSekolah.length > 0 && (
                <tfoot className="sticky bottom-0 z-10 shadow-[0_-4px_10px_rgba(0,0,0,0.04)]">
                  <tr className="bg-blue-100 text-center font-black uppercase text-xs border-t-2 border-blue-200">
                    <td colSpan="5" className="px-4 py-4 text-left rounded-l-2xl border-y border-l border-blue-200 text-blue-900">
                      TOTAL DARI {dataSekolah.length} SEKOLAH
                    </td>
                    <td className="px-4 py-4 text-blue-950 text-lg border-y border-r border-blue-200 rounded-r-2xl bg-blue-200/50">
                      {dataSekolah.length.toLocaleString()}
                    </td>
                  </tr>
                </tfoot>
              )}
