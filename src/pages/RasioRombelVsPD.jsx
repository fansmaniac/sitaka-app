...
import { MapPin, Info, Layers, Activity, Search, Download, Loader2, GraduationCap, School } from 'lucide-react';
...
// Fungsi hitung angka rasio mentah (Jumlah PD / Jumlah Rombel)
...
export default function RasioRombelVsPD({ selectedYear }) {
  ...
  const tab2Data = useMemo(() => {
    ...
    if (isModeSemua) {
      ...
      // Pada tingkat kecamatan, remap ke variabel penampung default (_rombel, _pd) agar seragam renderingnya
  ...
  const handleUnduhTab2 = async () => {
    const workbook = new ExcelJS.Workbook();
    const worksheet = workbook.addWorksheet('Analisa Rasio PD per Rombel');
    ...
    const link = document.createElement('a');
    link.href = URL.createObjectURL(blob);
    link.download = `Analisa_Rasio_PD_per_Rombel_${filterWilayah}_${filterStatusTab2}_${selectedYear}.xlsx`;
    link.click();
  };
