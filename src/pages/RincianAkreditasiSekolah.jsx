...
export default function RincianAkreditasiSekolah({ 
  ...
  const [filterStatus, setFilterStatus] = useState('SEMUA'); // Filter Negeri/Swasta
  ...
  // Engine Pemrosesan Data Akreditasi
  const processedData = useMemo(() => {
    ...
    validData.forEach(item => {
      ...
      if (!mapAgg.has(keyId)) {
        mapAgg.set(keyId, { namaWilayah: keyId, akr_a: 0, akr_b: 0, akr_c: 0, akr_belum: 0, total: 0 });
      }
  ...
  const columnTotals = useMemo(() => {
    return processedData.reduce((acc, curr) => {
      ...
      return acc;
    }, { akr_a: 0, akr_b: 0, akr_c: 0, akr_belum: 0, total: 0 });
  }, [processedData]);
  ...
  const downloadExcelRincian = async () => {
    ...
    worksheet.columns = [
      ...
      { header: 'Belum / Tidak Terakreditasi', key: 'akr_belum', width: 25 },
    ...
    const totalRow = worksheet.addRow({
      ...
      akr_a: columnTotals.akr_a,
      akr_b: columnTotals.akr_b,
      akr_c: columnTotals.akr_c,
      akr_belum: columnTotals.akr_belum,
    ...
    link.download = `Rincian_Akreditasi_${isModeSemua ? 'Provinsi' : filterWilayah}_${activeJenjang.replace('/','-')}_${filterStatus}.xlsx`;
    link.click();
  };
  ...
  return createPortal(
    <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-in fade-in duration-200" onClick={onClose}>
      <div className="bg-white w-full max-w-5xl h-[90vh] rounded-3xl shadow-2xl flex flex-col overflow-hidden animate-in zoom-in-95 duration-300" onClick={e => e.stopPropagation()}>
        ...
        {/* HEADER MODAL */}
        <div className="bg-emerald-700 px-6 py-5 flex items-center justify-between shrink-0">
          <div className="flex items-center gap-3 text-white">
            ...
            <div>
              ...
              <p className="text-emerald-200 text-sm font-bold uppercase tracking-widest mt-1 flex gap-2">
                ...
                <span className="text-emerald-100">Jenjang: {activeJenjang}</span>
              </p>
            </div>
          </div>
          <div className="flex items-center gap-3">
            ...
            <button onClick={onClose} className="p-2 bg-emerald-800 hover:bg-red-500 text-white rounded-xl transition-colors">
              <X size={24} />
            </button>
          </div>
        </div>
          ...
          <table className="w-full text-center border-separate border-spacing-y-2">
            <thead className="sticky top-0 bg-white z-10 shadow-sm rounded-xl">
              <tr className="text-[10px] font-black uppercase text-gray-500">
                ...
                <th className="px-3 py-3 text-red-600">Belum / TT</th>
            ...
            {processedData.length > 0 && (
              <tfoot className="sticky bottom-0 z-10 shadow-[0_-4px_10px_rgba(0,0,0,0.04)]">
                <tr className="bg-emerald-100 text-center font-black uppercase text-xs border-t-2 border-emerald-200">
                  ...
                  <td className="px-3 py-4 text-emerald-700 text-base border-y border-emerald-200">{columnTotals.akr_a.toLocaleString()}</td>
                  <td className="px-3 py-4 text-blue-700 text-base border-y border-emerald-200">{columnTotals.akr_b.toLocaleString()}</td>
                  <td className="px-3 py-4 text-amber-700 text-base border-y border-emerald-200">{columnTotals.akr_c.toLocaleString()}</td>
                  <td className="px-3 py-4 text-red-700 text-base border-y border-emerald-200">{columnTotals.akr_belum.toLocaleString()}</td>
