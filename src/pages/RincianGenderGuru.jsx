...
import { 
  Download, Users, MapPin, Search, X, ChevronLeft, ChevronRight, Building2 
} from 'lucide-react';
...
export default function RincianGenderGuru({ 
  ...
  const processedData = useMemo(() => {
    ...
    validData.forEach(item => {
      ...
      if (!mapAgg.has(keyId)) {
        mapAgg.set(keyId, { namaWilayah: keyId, gen_l: 0, gen_p: 0, total: 0 });
      }
  ...
  const columnTotals = useMemo(() => {
    return processedData.reduce((acc, curr) => {
      acc.gen_l += curr.gen_l;
      acc.gen_p += curr.gen_p;
      acc.total += curr.total;
      return acc;
    }, { gen_l: 0, gen_p: 0, total: 0 });
  }, [processedData]);
  ...
  const downloadExcelRincian = async () => {
    ...
    worksheet.columns = [
      ...
      { header: 'Guru Laki-laki', key: 'gen_l', width: 18 },
    ...
    const totalRow = worksheet.addRow({
      ...
      gen_l: columnTotals.gen_l,
      gen_p: columnTotals.gen_p,
      total: columnTotals.total
    });

    // Styling Header
    worksheet.getRow(1).font = { bold: true, color: { argb: 'FFFFFFFF' } };
    worksheet.getRow(1).fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FFBE185D' } }; // Pink 700
    ...
    link.download = `Rincian_Gender_Guru_${isModeSemua ? 'Provinsi' : filterWilayah}_${activeJenjang.replace('/','-')}_${filterStatus}.xlsx`;
    link.click();
  };
  ...
  return createPortal(
    <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-in fade-in duration-200" onClick={onClose}>
      <div className="bg-white w-full max-w-5xl h-[90vh] rounded-3xl shadow-2xl flex flex-col overflow-hidden animate-in zoom-in-95 duration-300" onClick={e => e.stopPropagation()}>
        ...
        {/* HEADER MODAL */}
        <div className="bg-pink-700 px-6 py-5 flex items-center justify-between shrink-0">
          ...
          <div className="flex items-center gap-3">
            ...
            <button onClick={onClose} className="p-2 bg-pink-800 hover:bg-red-500 text-white rounded-xl transition-colors">
              <X size={24} />
            </button>
          </div>
        </div>
          ...
          <table className="w-full text-center border-separate border-spacing-y-2">
            ...
            {processedData.length > 0 && (
              <tfoot className="sticky bottom-0 z-10 shadow-[0_-4px_10px_rgba(0,0,0,0.04)]">
                <tr className="bg-pink-50 text-center font-black uppercase text-xs border-t-2 border-pink-200">
                  ...
                  <td className="px-4 py-4 text-blue-800 text-base border-y border-pink-200">{columnTotals.gen_l.toLocaleString()}</td>
                  <td className="px-4 py-4 text-pink-800 text-base border-y border-pink-200">{columnTotals.gen_p.toLocaleString()}</td>
        ...
        <div className="bg-gray-50 p-4 border-t border-gray-200 flex items-center justify-between shrink-0 rounded-b-3xl">
          <div className="flex flex-col">
            <p className="text-xs font-bold text-gray-500">
              Menampilkan <span className="text-gray-800">{processedData.length === 0 ? 0 : startIndex + 1}</span> - <span className="text-gray-800">{Math.min(startIndex + rowsPerPage, processedData.length)}</span> dari <span className="text-pink-700 font-black">{processedData.length}</span> baris
            </p>
            {displayLastUpdated && (
          ...
          <div className="flex items-center gap-2">
            <button disabled={currentPage === 1} onClick={() => goToPage(currentPage - 1)} className="p-2 rounded-xl bg-white border border-gray-200 hover:bg-pink-50 disabled:opacity-50 transition-colors"><ChevronLeft size={16} /></button>
            <span className="text-xs font-black text-gray-600 px-2">Hal {currentPage} / {totalPages}</span>
            <button disabled={currentPage === totalPages || totalPages === 0} onClick={() => goToPage(currentPage + 1)} className="p-2 rounded-xl bg-white border border-gray-200 hover:bg-pink-50 disabled:opacity-50 transition-colors"><ChevronRight size={16} /></button>
          </div>
        </div>
