      ...
      <div className="bg-white w-full max-w-6xl h-[90vh] rounded-3xl shadow-2xl flex flex-col overflow-hidden animate-in zoom-in-95 duration-300" onClick={e => e.stopPropagation()}>
          ...
          <div className="flex items-center gap-3 text-white">
            ...
            <div>
              <h2 className="text-xl font-black uppercase tracking-tighter leading-none">
                Rincian Guru Berdasarkan Gender
              </h2>
        ...
        <div className="bg-pink-50 px-6 pt-3 flex gap-2 border-b border-pink-100 shrink-0">
          <button 
            ...
            className={`px-6 py-2.5 rounded-t-xl font-black uppercase text-xs transition-all border-b-4 ${activeModalTab === 'KECAMATAN' ? 'bg-white text-pink-700 border-pink-700 shadow-[0_-4px_10px_rgba(0,0,0,0.05)]' : 'bg-transparent text-pink-500 border-transparent hover:text-pink-700 hover:bg-pink-100/50'}`}
          >
            <div className="flex items-center gap-2"><MapPin size={16}/> Per Kecamatan</div>
          </button>
          <button 
            onClick={() => setActiveModalTab('SEKOLAH')}
            className={`px-6 py-2.5 rounded-t-xl font-black uppercase text-xs transition-all border-b-4 ${activeModalTab === 'SEKOLAH' ? 'bg-white text-pink-700 border-pink-700 shadow-[0_-4px_10px_rgba(0,0,0,0.05)]' : 'bg-transparent text-pink-500 border-transparent hover:text-pink-700 hover:bg-pink-100/50'}`}
          >
          ...
          <div className="bg-white px-6 pt-4 pb-0 flex gap-2 overflow-x-auto scrollbar-hide shrink-0 z-10 relative">
            ...
            {Object.keys(JENJANG_GROUPS).map(j => (
              <button 
                ...
                className={`px-5 py-2 rounded-xl font-black uppercase text-[10px] md:text-xs transition-all whitespace-nowrap border ${activeJenjangTab === j ? 'bg-pink-600 text-white border-pink-600 shadow-md' : 'bg-gray-50 text-gray-500 border-gray-200 hover:bg-gray-100'}`}
              >
                {j}
              </button>
            ))}
