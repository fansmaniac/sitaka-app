...
import { 
  Download, Briefcase, MapPin, Search, X, ChevronLeft, ChevronRight, Building2 
} from 'lucide-react';
...
export default function RincianKepegawaianGuru({ 
  ...
  // Proses Agregasi Data Kepegawaian
  ...
  const downloadExcelRincian = async () => {
    ...
    const totalRow = worksheet.addRow({
      ...
      peg_pns: columnTotals.peg_pns,
      peg_pppk: columnTotals.peg_pppk,
      peg_gty: columnTotals.peg_gty,
      peg_honor: columnTotals.peg_honor,
      peg_lain: columnTotals.peg_lain,
    ...
    link.download = `Rincian_Kepegawaian_Guru_${isModeSemua ? 'Provinsi' : filterWilayah}_${activeJenjang.replace('/','-')}_${filterStatus}.xlsx`;
    link.click();
  };
  ...
  return createPortal(
    <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-in fade-in duration-200" onClick={onClose}>
      <div className="bg-white w-full max-w-6xl h-[90vh] rounded-3xl shadow-2xl flex flex-col overflow-hidden animate-in zoom-in-95 duration-300" onClick={e => e.stopPropagation()}>
        ...
        {/* HEADER MODAL */}
        <div className="bg-teal-700 px-6 py-5 flex items-center justify-between shrink-0">
              ...
              {currentRows.map((row, idx) => (
                <tr key={idx} className="bg-white shadow-sm hover:shadow-md hover:scale-[1.01] transition-all group">
                  ...
                  <td className="px-3 py-4 font-black text-blue-600 text-base border-y border-gray-100 bg-blue-50/30">
                    {row.peg_pns.toLocaleString()}
                  </td>
                  <td className="px-3 py-4 font-black text-emerald-600 text-base border-y border-gray-100 bg-emerald-50/30">
                    {row.peg_pppk.toLocaleString()}
                  </td>
                  <td className="px-3 py-4 font-black text-orange-600 text-base border-y border-gray-100 bg-orange-50/30">
                    {row.peg_gty.toLocaleString()}
                  </td>
                  <td className="px-3 py-4 font-black text-red-600 text-base border-y border-gray-100 bg-red-50/30">
                    {row.peg_honor.toLocaleString()}
                  </td>
                  <td className="px-3 py-4 font-black text-gray-500 text-base border-y border-gray-100 bg-gray-50/50">
                    {row.peg_lain.toLocaleString()}
                  </td>
                  <td className="px-4 py-4 font-black text-teal-800 text-lg border-y border-r border-gray-100 bg-teal-50/50 rounded-r-2xl">
            ...
            {processedData.length > 0 && (
              <tfoot className="sticky bottom-0 z-10 shadow-[0_-4px_10px_rgba(0,0,0,0.04)]">
                <tr className="bg-teal-100 text-center font-black uppercase text-xs border-t-2 border-teal-200">
                  ...
                  <td className="px-3 py-4 text-blue-800 text-base border-y border-teal-200">{columnTotals.peg_pns.toLocaleString()}</td>
                  <td className="px-3 py-4 text-emerald-800 text-base border-y border-teal-200">{columnTotals.peg_pppk.toLocaleString()}</td>
                  <td className="px-3 py-4 text-orange-800 text-base border-y border-teal-200">{columnTotals.peg_gty.toLocaleString()}</td>
                  <td className="px-3 py-4 text-red-800 text-base border-y border-teal-200">{columnTotals.peg_honor.toLocaleString()}</td>
                  <td className="px-3 py-4 text-gray-800 text-base border-y border-teal-200">{columnTotals.peg_lain.toLocaleString()}</td>
        ...
        <div className="bg-gray-50 p-4 border-t border-gray-200 flex items-center justify-between shrink-0 rounded-b-3xl">
          <div className="flex flex-col">
            <p className="text-xs font-bold text-gray-500">
              Menampilkan <span className="text-gray-800">{processedData.length === 0 ? 0 : startIndex + 1}</span> - <span className="text-gray-800">{Math.min(startIndex + rowsPerPage, processedData.length)}</span> dari <span className="text-teal-700 font-black">{processedData.length}</span> baris
            </p>
            {displayLastUpdated && (
          ...
          <div className="flex items-center gap-2">
            <button disabled={currentPage === 1} onClick={() => goToPage(currentPage - 1)} className="p-2 rounded-xl bg-white border border-gray-200 hover:bg-teal-50 disabled:opacity-50 transition-colors"><ChevronLeft size={16} /></button>
            <span className="text-xs font-black text-gray-600 px-2">Hal {currentPage} / {totalPages}</span>
            <button disabled={currentPage === totalPages || totalPages === 0} onClick={() => goToPage(currentPage + 1)} className="p-2 rounded-xl bg-white border border-gray-200 hover:bg-teal-50 disabled:opacity-50 transition-colors"><ChevronRight size={16} /></button>
          </div>
        </div>
