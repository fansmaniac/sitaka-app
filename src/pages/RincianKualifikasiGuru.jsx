...
import { 
  Download, GraduationCap, MapPin, Search, X, ChevronLeft, ChevronRight, 
  Building2, School
} from 'lucide-react';
...
// =====================================================================
// MAIN COMPONENT: RINCIAN KUALIFIKASI GURU
// =====================================================================
export default function RincianKualifikasiGuru({ 
  isOpen, 
  onClose, 
  data = [], 
  ...
  const dataKecamatan = useMemo(() => {
    ...
    baseData.forEach(item => {
      ...
      if (!mapAgg.has(keyId)) {
        mapAgg.set(keyId, { namaWilayah: keyId, kual_s1: 0, kual_s2: 0, kual_kurang: 0, kual_lain: 0, total: 0 });
      }
  ...
  const totalsKecamatan = useMemo(() => {
    return dataKecamatan.reduce((acc, curr) => {
      acc.kual_s1 += curr.kual_s1;
      acc.kual_s2 += curr.kual_s2;
      acc.kual_kurang += curr.kual_kurang;
      acc.kual_lain += curr.kual_lain;
      acc.total += curr.total;
      return acc;
    }, { kual_s1: 0, kual_s2: 0, kual_kurang: 0, kual_lain: 0, total: 0 });
  }, [dataKecamatan]);
  ...
  const downloadExcelRincian = async () => {
    ...
    if (activeModalTab === 'KECAMATAN') {
      const worksheet = workbook.addWorksheet('Rekap Kualifikasi Wilayah');
      worksheet.columns = [
        { header: isModeSemua ? 'Kabupaten/Kota' : 'Kecamatan', key: 'namaWilayah', width: 30 },
        { header: 'S1 / D4', key: 'kual_s1', width: 15 },
        { header: 'S2 / S3', key: 'kual_s2', width: 15 },
        { header: '< S1', key: 'kual_kurang', width: 15 },
        { header: 'Lainnya', key: 'kual_lain', width: 15 },
        { header: 'Total Guru', key: 'total', width: 18 },
      ];
      dataKecamatan.forEach(item => worksheet.addRow(item));
      const totalRow = worksheet.addRow({ namaWilayah: 'TOTAL', kual_s1: totalsKecamatan.kual_s1, kual_s2: totalsKecamatan.kual_s2, kual_kurang: totalsKecamatan.kual_kurang, kual_lain: totalsKecamatan.kual_lain, total: totalsKecamatan.total });
      worksheet.getRow(1).font = { bold: true, color: { argb: 'FFFFFFFF' } };
      worksheet.getRow(1).fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FF7E22CE' } };
      const buffer = await workbook.xlsx.writeBuffer();
      const blob = new Blob([buffer], { type: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet' });
      const link = document.createElement('a');
      link.href = URL.createObjectURL(blob);
      link.download = `Rincian_Guru_Kualifikasi_${initialWilayah}_${safeName}.xlsx`;
      link.click();
    } else {
      const worksheet = workbook.addWorksheet('Daftar Sekolah & Kualifikasi');
      worksheet.columns = [
      ...
      const link = document.createElement('a');
      link.href = URL.createObjectURL(blob);
      link.download = `Daftar_Guru_Sekolah_Kualifikasi_${initialWilayah}_${safeName}.xlsx`;
      link.click();
    }
  };
      ...
      <div className="bg-white w-full max-w-7xl h-[90vh] rounded-3xl shadow-2xl flex flex-col overflow-hidden animate-in zoom-in-95 duration-300" onClick={e => e.stopPropagation()}>
        ...
        <div className="bg-purple-700 px-6 py-5 flex items-center justify-between shrink-0">
          <div className="flex items-center gap-3 text-white">
            ...
            <div>
              <h2 className="text-xl font-black uppercase tracking-tighter leading-none">
                Rincian Kualifikasi Pendidikan Guru
              </h2>
              <p className="text-purple-200 text-sm font-bold uppercase tracking-widest mt-1 flex gap-2">
          ...
          <div className="flex items-center gap-3">
            ...
            <button onClick={onClose} className="p-2 bg-purple-800 hover:bg-red-500 text-white rounded-xl transition-colors">
              <X size={24} />
            </button>
          </div>
        </div>
        ...
        <div className="bg-purple-50 px-6 pt-3 flex gap-2 border-b border-purple-100 shrink-0">
          <button 
            onClick={() => setActiveModalTab('KECAMATAN')}
            className={`px-6 py-2.5 rounded-t-xl font-black uppercase text-xs transition-all border-b-4 ${activeModalTab === 'KECAMATAN' ? 'bg-white text-purple-700 border-purple-700 shadow-[0_-4px_10px_rgba(0,0,0,0.05)]' : 'bg-transparent text-purple-500 border-transparent hover:text-purple-700 hover:bg-purple-100/50'}`}
          >
            <div className="flex items-center gap-2"><MapPin size={16}/> Per Kecamatan</div>
          </button>
          <button 
            onClick={() => setActiveModalTab('SEKOLAH')}
            className={`px-6 py-2.5 rounded-t-xl font-black uppercase text-xs transition-all border-b-4 ${activeModalTab === 'SEKOLAH' ? 'bg-white text-purple-700 border-purple-700 shadow-[0_-4px_10px_rgba(0,0,0,0.05)]' : 'bg-transparent text-purple-500 border-transparent hover:text-purple-700 hover:bg-purple-100/50'}`}
          >
        ...
        {activeModalTab === 'SEKOLAH' && (
          <div className="bg-white px-6 pt-4 pb-0 flex gap-2 overflow-x-auto scrollbar-hide shrink-0 z-10 relative">
            <button 
              ...
              className={`px-5 py-2 rounded-xl font-black uppercase text-[10px] md:text-xs transition-all whitespace-nowrap border ${activeJenjangTab === mappedJenjang ? 'bg-purple-700 text-white border-purple-700 shadow-md' : 'bg-gray-50 text-gray-500 border-gray-200 hover:bg-gray-100'}`}
            >
            ...
            {availableTabs.map(j => (
              <button 
                ...
                className={`px-5 py-2 rounded-xl font-black uppercase text-[10px] md:text-xs transition-all whitespace-nowrap border ${activeJenjangTab === j ? 'bg-purple-700 text-white border-purple-700 shadow-md' : 'bg-gray-50 text-gray-500 border-gray-200 hover:bg-gray-100'}`}
              >
                {j}
              </button>
            ))}
        ...
        <div className={`bg-white px-6 py-4 border-b border-gray-200 flex flex-wrap gap-4 items-center shrink-0 shadow-sm z-10 ${activeModalTab === 'SEKOLAH' ? 'border-t-0 pt-3' : ''}`}>
          <div className="relative flex-1 min-w-[250px]">
            ...
            <input 
              ...
              className="w-full pl-10 pr-10 py-2.5 rounded-xl border border-gray-200 outline-none focus:border-purple-500 focus:ring-2 focus:ring-purple-200 font-bold text-gray-700"
            />
          </div>
            ...
            <table className="w-full text-center border-separate border-spacing-y-2">
              <thead className="sticky top-0 bg-white z-10 shadow-sm rounded-xl">
                <tr className="text-[10px] font-black uppercase text-gray-500">
                  ...
                  <th className="px-4 py-3 text-emerald-600">S1 / D4</th>
                  <th className="px-4 py-3 text-blue-600">S2 / S3</th>
                  <th className="px-4 py-3 text-amber-600">&lt; S1</th>
                  <th className="px-4 py-3 text-gray-500">Lainnya</th>
              ...
              <tbody>
                {currentRows.map((row, idx) => (
                  <tr key={idx} className="bg-white shadow-sm hover:shadow-md hover:scale-[1.01] transition-all group">
                    ...
                    <td className="px-4 py-4 font-black text-emerald-600 text-base border-y border-gray-100 bg-emerald-50/30">
                      {row.kual_s1.toLocaleString()}
                    </td>
                    <td className="px-4 py-4 font-black text-blue-600 text-base border-y border-gray-100 bg-blue-50/30">
                      {row.kual_s2.toLocaleString()}
                    </td>
                    <td className="px-4 py-4 font-black text-amber-600 text-base border-y border-gray-100 bg-amber-50/30">
                      {row.kual_kurang.toLocaleString()}
                    </td>
                    <td className="px-4 py-4 font-black text-gray-500 text-base border-y border-gray-100 bg-gray-50/50">
                      {row.kual_lain.toLocaleString()}
                    </td>
                    <td className="px-4 py-4 font-black text-purple-800 text-lg border-y border-r border-gray-100 bg-purple-50/50 rounded-r-2xl">
              ...
              {dataKecamatan.length > 0 && (
                <tfoot className="sticky bottom-0 z-10 shadow-[0_-4px_10px_rgba(0,0,0,0.04)]">
                  <tr className="bg-purple-100 text-center font-black uppercase text-xs border-t-2 border-purple-200">
                    ...
                    <td className="px-4 py-4 text-emerald-800 text-base border-y border-purple-200">{totalsKecamatan.kual_s1.toLocaleString()}</td>
                    <td className="px-4 py-4 text-blue-800 text-base border-y border-purple-200">{totalsKecamatan.kual_s2.toLocaleString()}</td>
                    <td className="px-4 py-4 text-amber-800 text-base border-y border-purple-200">{totalsKecamatan.kual_kurang.toLocaleString()}</td>
                    <td className="px-4 py-4 text-gray-800 text-base border-y border-purple-200">{totalsKecamatan.kual_lain.toLocaleString()}</td>
                    <td className="px-4 py-4 text-purple-950 text-lg border-y border-r border-purple-200 rounded-r-2xl bg-purple-200/50">
            ...
            <table className="w-full text-center border-separate border-spacing-y-2">
              <thead className="sticky top-0 bg-white z-10 shadow-sm rounded-xl">
                <tr className="text-[10px] font-black uppercase text-gray-500">
                  ...
                  <th className="px-3 py-3 text-purple-600">Jenjang</th>
              ...
              <tbody>
                {currentRows.map((row, idx) => (
                  <tr key={idx} className="bg-white shadow-sm hover:shadow-md hover:scale-[1.01] transition-all group">
                    ...
                    <td className="px-3 py-3 font-bold text-purple-600 text-xs border-y border-gray-100 uppercase">{row.jenjang}</td>
                    <td className="px-3 py-3 font-bold text-orange-600 text-xs border-y border-gray-100 uppercase">{row.status}</td>
                    
                    <td className="px-4 py-3 font-bold text-emerald-600 text-sm border-y border-gray-100 bg-emerald-50/20">{row.kual_s1.toLocaleString()}</td>
                    <td className="px-4 py-3 font-bold text-blue-600 text-sm border-y border-gray-100 bg-blue-50/20">{row.kual_s2.toLocaleString()}</td>
                    <td className="px-4 py-3 font-bold text-amber-600 text-sm border-y border-gray-100 bg-amber-50/20">{row.kual_kurang.toLocaleString()}</td>
                    <td className="px-4 py-3 font-bold text-gray-500 text-sm border-y border-gray-100 bg-gray-50/50">{row.kual_lain.toLocaleString()}</td>

                    <td className="px-4 py-3 font-black text-purple-800 text-lg border-y border-r border-gray-100 bg-purple-50/50 rounded-r-2xl">
                      {row.total.toLocaleString()}
                    </td>
                  </tr>
                ))}
              ...
              {dataSekolah.length > 0 && (
                <tfoot className="sticky bottom-0 z-10 shadow-[0_-4px_10px_rgba(0,0,0,0.04)]">
                  <tr className="bg-purple-100 text-center font-black uppercase text-xs border-t-2 border-purple-200">
                    <td colSpan="6" className="px-4 py-4 text-left rounded-l-2xl border-y border-l border-purple-200 text-purple-900">
                      TOTAL GURU DARI {dataSekolah.length} SEKOLAH
                    </td>
                    <td className="px-4 py-4 text-emerald-800 text-base border-y border-purple-200">{totalGuruSekolah.kual_s1.toLocaleString()}</td>
                    <td className="px-4 py-4 text-blue-800 text-base border-y border-purple-200">{totalGuruSekolah.kual_s2.toLocaleString()}</td>
                    <td className="px-4 py-4 text-amber-800 text-base border-y border-purple-200">{totalGuruSekolah.kual_kurang.toLocaleString()}</td>
                    <td className="px-4 py-4 text-gray-800 text-base border-y border-purple-200">{totalGuruSekolah.kual_lain.toLocaleString()}</td>
                    <td className="px-4 py-4 text-purple-950 text-lg border-y border-r border-purple-200 rounded-r-2xl bg-purple-200/50">
        ...
        <div className="bg-white p-4 border-t border-gray-200 flex items-center justify-between shrink-0 rounded-b-3xl">
          <div className="flex flex-col">
            <p className="text-xs font-bold text-gray-500">
              Menampilkan <span className="text-gray-800">{activeData.length === 0 ? 0 : startIndex + 1}</span> - <span className="text-gray-800">{Math.min(startIndex + rowsPerPage, activeData.length)}</span> dari <span className="text-purple-700 font-black">{activeData.length}</span> baris
            </p>
            {displayLastUpdated && (
          ...
          <div className="flex items-center gap-2">
            <button disabled={currentPage === 1} onClick={() => goToPage(currentPage - 1)} className="p-2 rounded-xl bg-gray-50 border border-gray-200 hover:bg-purple-50 disabled:opacity-50 transition-colors"><ChevronLeft size={16} /></button>
            <span className="text-xs font-black text-gray-600 px-2">Hal {currentPage} / {totalPages}</span>
            <button disabled={currentPage === totalPages || totalPages === 0} onClick={() => goToPage(currentPage + 1)} className="p-2 rounded-xl bg-gray-50 border border-gray-200 hover:bg-purple-50 disabled:opacity-50 transition-colors"><ChevronRight size={16} /></button>
          </div>
        </div>
