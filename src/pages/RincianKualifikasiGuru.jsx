...
import { 
  Download, GraduationCap, MapPin, Search, X, ChevronLeft, ChevronRight, Building2 
} from 'lucide-react';
...
export default function RincianKualifikasiGuru({ 
  ...
  // Proses Agregasi Data Kualifikasi
  ...
  const downloadExcelRincian = async () => {
    ...
    const totalRow = worksheet.addRow({
      ...
      kual_s1: columnTotals.kual_s1,
      kual_s2: columnTotals.kual_s2,
      kual_kurang: columnTotals.kual_kurang,
      kual_lain: columnTotals.kual_lain,
    ...
    link.download = `Rincian_Kualifikasi_Guru_${isModeSemua ? 'Provinsi' : filterWilayah}_${activeJenjang.replace('/','-')}_${filterStatus}.xlsx`;
    link.click();
  };
            ...
            {processedData.length > 0 && (
              <tfoot className="sticky bottom-0 z-10 shadow-[0_-4px_10px_rgba(0,0,0,0.04)]">
                <tr className="bg-purple-100 text-center font-black uppercase text-xs border-t-2 border-purple-200">
                  ...
                  <td className="px-4 py-4 text-emerald-800 text-base border-y border-purple-200">{columnTotals.kual_s1.toLocaleString()}</td>
                  <td className="px-4 py-4 text-blue-800 text-base border-y border-purple-200">{columnTotals.kual_s2.toLocaleString()}</td>
                  <td className="px-4 py-4 text-amber-800 text-base border-y border-purple-200">{columnTotals.kual_kurang.toLocaleString()}</td>
                  <td className="px-4 py-4 text-gray-800 text-base border-y border-purple-200">{columnTotals.kual_lain.toLocaleString()}</td>
        ...
        <div className="bg-gray-50 p-4 border-t border-gray-200 flex items-center justify-between shrink-0 rounded-b-3xl">
          <div className="flex flex-col">
            <p className="text-xs font-bold text-gray-500">
              Menampilkan <span className="text-gray-800">{processedData.length === 0 ? 0 : startIndex + 1}</span> - <span className="text-gray-800">{Math.min(startIndex + rowsPerPage, processedData.length)}</span> dari <span className="text-purple-700 font-black">{processedData.length}</span> baris
            </p>
            {displayLastUpdated && (
          ...
          <div className="flex items-center gap-2">
            <button disabled={currentPage === 1} onClick={() => goToPage(currentPage - 1)} className="p-2 rounded-xl bg-white border border-gray-200 hover:bg-purple-50 disabled:opacity-50 transition-colors"><ChevronLeft size={16} /></button>
            <span className="text-xs font-black text-gray-600 px-2">Hal {currentPage} / {totalPages}</span>
            <button disabled={currentPage === totalPages || totalPages === 0} onClick={() => goToPage(currentPage + 1)} className="p-2 rounded-xl bg-white border border-gray-200 hover:bg-purple-50 disabled:opacity-50 transition-colors"><ChevronRight size={16} /></button>
          </div>
        </div>
