  ...
  const downloadExcelRincian = async () => {
    ...
    if (activeModalTab === 'KECAMATAN') {
      ...
      const totalRow = worksheet.addRow({
        ...
        kual_s1: totalsKecamatan.kual_s1,
        kual_s2: totalsKecamatan.kual_s2,
        kual_kurang: totalsKecamatan.kual_kurang,
        kual_lain: totalsKecamatan.kual_lain,
        total: totalsKecamatan.total
      });

      worksheet.getRow(1).font = { bold: true, color: { argb: 'FFFFFFFF' } };
      worksheet.getRow(1).fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FF7E22CE' } }; // Purple 700
      ...
      link.download = `Rincian_Kualifikasi_Kecamatan_${initialWilayah}.xlsx`;
      link.click();
    } else {
      // Tab SEKOLAH
      ...
      const link = document.createElement('a');
      link.href = URL.createObjectURL(blob);
      link.download = `Daftar_Sekolah_Kualifikasi_${initialWilayah}_${activeJenjangTab.replace(' ','_')}.xlsx`;
      link.click();
    }
  };
        ...
        {activeModalTab === 'SEKOLAH' && (
          <div className="bg-white px-6 pt-4 pb-0 flex gap-2 overflow-x-auto scrollbar-hide shrink-0 z-10 relative">
            <button 
              ...
              className={`px-5 py-2 rounded-xl font-black uppercase text-[10px] md:text-xs transition-all whitespace-nowrap border ${activeJenjangTab === 'SEMUA' ? 'bg-purple-700 text-white border-purple-700 shadow-md' : 'bg-gray-50 text-gray-500 border-gray-200 hover:bg-gray-100'}`}
            >
              Semua Jenjang
            </button>
