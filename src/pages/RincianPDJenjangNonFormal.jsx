...
import { 
  Download, Tent, MapPin, Search, X, ChevronLeft, ChevronRight 
} from 'lucide-react';
...
const JENJANG_GROUPS = {
  'NON FORMAL': ['PKBM', 'SKB', 'SPS', 'TPA'] // Sesuai dengan filter NON FORMAL utama
};

const identifyJenjangGroup = (jenjangDb) => {
  const j = String(jenjangDb).trim().toUpperCase();
  if (JENJANG_GROUPS['NON FORMAL'].includes(j)) return 'NON FORMAL';
  return null;
};
...
// MAIN COMPONENT
// =====================================================================
export default function RincianPDJenjangNonFormal({ 
  isOpen, 
  onClose, 
  data = [], 
  ...
  // Proses Agregasi Data Khusus Non Formal
  const processedData = useMemo(() => {
    if (!data) return [];

    const validData = data.filter(item => {
      // Filter khusus Non Formal
      const group = identifyJenjangGroup(getVal(item, 'bentuk_pendidikan') || getVal(item, 'jenjang'));
      if (group !== 'NON FORMAL') return false;
    ...
    validData.forEach(item => {
      ...
      if (!mapAgg.has(keyId)) {
        mapAgg.set(keyId, { namaWilayah: keyId, nf_l: 0, nf_p: 0, total: 0 });
      }
  ...
  const downloadExcelRincian = async () => {
    ...
    worksheet.columns = [
      ...
      { header: 'Laki-Laki', key: 'nf_l', width: 18 },
      { header: 'Perempuan', key: 'nf_p', width: 18 },
    ...
    const totalRow = worksheet.addRow({
      ...
      nf_l: columnTotals.nf_l,
      nf_p: columnTotals.nf_p,
    ...
    link.download = `Rincian_PD_Non_Formal_${isModeSemua ? 'Provinsi' : filterWilayah}.xlsx`;
    link.click();
  };
      ...
      <div className="bg-white w-full max-w-5xl h-[90vh] rounded-3xl shadow-2xl flex flex-col overflow-hidden animate-in zoom-in-95 duration-300" onClick={e => e.stopPropagation()}>
        ...
        <div className="bg-teal-600 px-6 py-5 flex items-center justify-between shrink-0">
          <div className="flex items-center gap-3 text-white">
            <div className="bg-white/20 p-2 rounded-xl"><Tent size={24} /></div>
            <div>
              <h2 className="text-xl font-black uppercase tracking-tighter leading-none">
                Rincian Peserta Didik Non Formal
              </h2>
        ...
        <div className="flex-1 overflow-auto bg-white p-4">
            ...
            {processedData.length > 0 && (
              <tfoot className="sticky bottom-0 z-10 shadow-[0_-4px_10px_rgba(0,0,0,0.04)]">
                <tr className="bg-teal-100 text-center font-black uppercase text-xs border-t-2 border-teal-200">
                  ...
                  <td className="px-4 py-4 text-sky-800 text-base border-y border-teal-200">{columnTotals.nf_l.toLocaleString()}</td>
                  <td className="px-4 py-4 text-pink-800 text-base border-y border-teal-200">{columnTotals.nf_p.toLocaleString()}</td>
          ...
          {processedData.length === 0 && (
             <div className="py-20 flex flex-col items-center opacity-30 text-gray-500">
               ...
               <p className="font-black uppercase tracking-widest text-xl">Tidak Ada Data Non Formal</p>
             </div>
          )}
        </div>
