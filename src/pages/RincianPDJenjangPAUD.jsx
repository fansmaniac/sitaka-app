...
const JENJANG_GROUPS = {
  'PAUD': ['TK', 'KB', 'PAUD'] // Sesuai dengan filter PAUD utama
};

const identifyJenjangGroup = (jenjangDb) => {
  const j = String(jenjangDb).trim().toUpperCase();
  if (JENJANG_GROUPS['PAUD'].includes(j)) return 'PAUD';
  return null;
};
...
export default function RincianPDJenjangPAUD({ 
  ...
  // Proses Agregasi Data Khusus PAUD
  ...
  const downloadExcelRincian = async () => {
    ...
    const totalRow = worksheet.addRow({
      ...
      paud_l: columnTotals.paud_l,
      paud_p: columnTotals.paud_p,
    ...
    link.download = `Rincian_PD_PAUD_${isModeSemua ? 'Provinsi' : filterWilayah}.xlsx`;
    link.click();
  };
            ...
            {processedData.length > 0 && (
              <tfoot className="sticky bottom-0 z-10 shadow-[0_-4px_10px_rgba(0,0,0,0.04)]">
                <tr className="bg-pink-100 text-center font-black uppercase text-xs border-t-2 border-pink-200">
                  ...
                  <td className="px-4 py-4 text-sky-800 text-base border-y border-pink-200">{columnTotals.paud_l.toLocaleString()}</td>
                  <td className="px-4 py-4 text-pink-800 text-base border-y border-pink-200">{columnTotals.paud_p.toLocaleString()}</td>
