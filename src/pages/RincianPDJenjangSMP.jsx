...
export default function RincianPDJenjangSMP({ 
  ...
  // Proses Agregasi Data Khusus SMP
  ...
  const downloadExcelRincian = async () => {
    ...
    const totalRow = worksheet.addRow({
      ...
      smp_7: columnTotals.smp_7, 
      smp_8: columnTotals.smp_8, 
      smp_9: columnTotals.smp_9,
    ...
    link.download = `Rincian_PD_SMP_${isModeSemua ? 'Provinsi' : filterWilayah}.xlsx`;
    link.click();
  };
            ...
            {processedData.length > 0 && (
              <tfoot className="sticky bottom-0 z-10 shadow-[0_-4px_10px_rgba(0,0,0,0.04)]">
                <tr className="bg-indigo-100 text-center font-black uppercase text-xs border-t-2 border-indigo-200">
                  ...
                  <td className="px-4 py-4 text-indigo-600 border-y border-indigo-200 text-base">{columnTotals.smp_7.toLocaleString()}</td>
                  <td className="px-4 py-4 text-violet-600 border-y border-indigo-200 text-base">{columnTotals.smp_8.toLocaleString()}</td>
                  <td className="px-4 py-4 text-fuchsia-600 border-y border-indigo-200 text-base">{columnTotals.smp_9.toLocaleString()}</td>
        ...
        <div className="bg-gray-50 p-4 border-t border-gray-200 flex items-center justify-between shrink-0 rounded-b-3xl">
          <div className="flex flex-col">
            <p className="text-xs font-bold text-gray-500">
              Menampilkan <span className="text-gray-800">{processedData.length === 0 ? 0 : startIndex + 1}</span> - <span className="text-gray-800">{Math.min(startIndex + rowsPerPage, processedData.length)}</span> dari <span className="text-indigo-700 font-black">{processedData.length}</span> baris
            </p>
            {displayLastUpdated && (
          ...
          <div className="flex items-center gap-2">
            <button disabled={currentPage === 1} onClick={() => goToPage(currentPage - 1)} className="p-2 rounded-xl bg-white border border-gray-200 hover:bg-indigo-50 disabled:opacity-50 transition-colors"><ChevronLeft size={16} /></button>
            <span className="text-xs font-black text-gray-600 px-2">Hal {currentPage} / {totalPages}</span>
            <button disabled={currentPage === totalPages || totalPages === 0} onClick={() => goToPage(currentPage + 1)} className="p-2 rounded-xl bg-white border border-gray-200 hover:bg-indigo-50 disabled:opacity-50 transition-colors"><ChevronRight size={16} /></button>
          </div>
        </div>
