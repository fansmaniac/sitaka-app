...
import { 
  Download, Award, MapPin, Search, X, ChevronLeft, ChevronRight, Building2 
} from 'lucide-react';
...
export default function RincianProfesiGuru({ 
  ...
  // Proses Agregasi Data Profesi/Sertifikasi
  ...
  const downloadExcelRincian = async () => {
    ...
    const totalRow = worksheet.addRow({
      ...
      sert_sudah: columnTotals.sert_sudah,
      sert_belum: columnTotals.sert_belum,
      total: columnTotals.total
    });

    // Styling Header
    worksheet.getRow(1).font = { bold: true, color: { argb: 'FFFFFFFF' } };
    worksheet.getRow(1).fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FFB45309' } }; // Amber 700
    ...
    link.download = `Rincian_Profesi_Guru_${isModeSemua ? 'Provinsi' : filterWilayah}_${activeJenjang.replace('/','-')}_${filterStatus}.xlsx`;
    link.click();
  };
  ...
  return createPortal(
    <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-in fade-in duration-200" onClick={onClose}>
      <div className="bg-white w-full max-w-5xl h-[90vh] rounded-3xl shadow-2xl flex flex-col overflow-hidden animate-in zoom-in-95 duration-300" onClick={e => e.stopPropagation()}>
        ...
        {/* HEADER MODAL */}
        <div className="bg-amber-700 px-6 py-5 flex items-center justify-between shrink-0">
          <div className="flex items-center gap-3 text-white">
            <div className="bg-white/20 p-2 rounded-xl"><Award size={24} /></div>
            <div>
              <h2 className="text-xl font-black uppercase tracking-tighter leading-none">
                Rincian Profesi / Sertifikasi Guru
              </h2>
              <p className="text-amber-200 text-sm font-bold uppercase tracking-widest mt-1 flex gap-2">
                ...
                <span className="text-yellow-200">Jenjang: {activeJenjang}</span>
              </p>
            </div>
          </div>
          ...
          <table className="w-full text-center border-separate border-spacing-y-2">
            <thead className="sticky top-0 bg-white z-10 shadow-sm rounded-xl">
              <tr className="text-[10px] font-black uppercase text-gray-500">
                ...
                <th className="px-4 py-3 text-emerald-600">Sudah Sertifikasi</th>
                <th className="px-4 py-3 text-red-600">Belum Sertifikasi</th>
              ...
              {currentRows.map((row, idx) => (
                <tr key={idx} className="bg-white shadow-sm hover:shadow-md hover:scale-[1.01] transition-all group">
                  ...
                  <td className="px-4 py-4 font-black text-emerald-600 text-base border-y border-gray-100 bg-emerald-50/30">
                    {row.sert_sudah.toLocaleString()}
                  </td>
                  <td className="px-4 py-4 font-black text-red-600 text-base border-y border-gray-100 bg-red-50/30">
                    {row.sert_belum.toLocaleString()}
                  </td>
                  <td className="px-4 py-4 font-black text-amber-800 text-lg border-y border-r border-gray-100 bg-amber-50/50 rounded-r-2xl">
            ...
            {processedData.length > 0 && (
              <tfoot className="sticky bottom-0 z-10 shadow-[0_-4px_10px_rgba(0,0,0,0.04)]">
                <tr className="bg-amber-100 text-center font-black uppercase text-xs border-t-2 border-amber-200">
                  ...
                  <td className="px-4 py-4 text-emerald-800 text-base border-y border-amber-200">{columnTotals.sert_sudah.toLocaleString()}</td>
                  <td className="px-4 py-4 text-red-800 text-base border-y border-amber-200">{columnTotals.sert_belum.toLocaleString()}</td>
        ...
        <div className="bg-gray-50 p-4 border-t border-gray-200 flex items-center justify-between shrink-0 rounded-b-3xl">
          <div className="flex flex-col">
            <p className="text-xs font-bold text-gray-500">
              Menampilkan <span className="text-gray-800">{processedData.length === 0 ? 0 : startIndex + 1}</span> - <span className="text-gray-800">{Math.min(startIndex + rowsPerPage, processedData.length)}</span> dari <span className="text-amber-700 font-black">{processedData.length}</span> baris
            </p>
            {displayLastUpdated && (
          ...
          <div className="flex items-center gap-2">
            <button disabled={currentPage === 1} onClick={() => goToPage(currentPage - 1)} className="p-2 rounded-xl bg-white border border-gray-200 hover:bg-amber-50 disabled:opacity-50 transition-colors"><ChevronLeft size={16} /></button>
            <span className="text-xs font-black text-gray-600 px-2">Hal {currentPage} / {totalPages}</span>
            <button disabled={currentPage === totalPages || totalPages === 0} onClick={() => goToPage(currentPage + 1)} className="p-2 rounded-xl bg-white border border-gray-200 hover:bg-amber-50 disabled:opacity-50 transition-colors"><ChevronRight size={16} /></button>
          </div>
        </div>
