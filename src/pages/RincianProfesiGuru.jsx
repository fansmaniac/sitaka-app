  ...
  const downloadExcelRincian = async () => {
    ...
    if (activeModalTab === 'KECAMATAN') {
      ...
      const totalRow = worksheet.addRow({
        ...
        sert_sudah: totalsKecamatan.sert_sudah,
        sert_belum: totalsKecamatan.sert_belum,
        total: totalsKecamatan.total
      });

      worksheet.getRow(1).font = { bold: true, color: { argb: 'FFFFFFFF' } };
      worksheet.getRow(1).fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FFD97706' } }; // Amber 600
      totalRow.font = { bold: true, color: { argb: 'FFB45309' } }; // Amber 900
      ...
      link.download = `Rincian_ProfesiGuru_Kecamatan_${initialWilayah}.xlsx`;
      link.click();
    } else {
      // Tab SEKOLAH
      ...
      const link = document.createElement('a');
      link.href = URL.createObjectURL(blob);
      link.download = `Daftar_Sekolah_ProfesiGuru_${initialWilayah}_${activeJenjangTab.replace(' ','_')}.xlsx`;
      link.click();
    }
  };
        ...
        {activeModalTab === 'SEKOLAH' && (
          <div className="bg-white px-6 pt-4 pb-0 flex gap-2 overflow-x-auto scrollbar-hide shrink-0 z-10 relative">
            <button 
              ...
              className={`px-5 py-2 rounded-xl font-black uppercase text-[10px] md:text-xs transition-all whitespace-nowrap border ${activeJenjangTab === 'SEMUA' ? 'bg-amber-600 text-white border-amber-600 shadow-md' : 'bg-gray-50 text-gray-500 border-gray-200 hover:bg-gray-100'}`}
            >
              Semua Jenjang
            </button>
