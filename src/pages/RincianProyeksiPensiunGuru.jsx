...
import { 
  Download, CalendarDays, MapPin, Search, X, ChevronLeft, ChevronRight, 
  Building2, School
} from 'lucide-react';
...
// =====================================================================
// MAIN COMPONENT: RINCIAN PROYEKSI PENSIUN GURU
// =====================================================================
export default function RincianProyeksiPensiunGuru({ 
  isOpen, 
  onClose, 
  data = [], 
  ...
  const dataKecamatan = useMemo(() => {
    ...
    baseData.forEach(item => {
      ...
      if (!mapAgg.has(keyId)) {
        mapAgg.set(keyId, { namaWilayah: keyId, pens_5: 0, pens_4: 0, pens_3: 0, pens_2: 0, pens_1: 0, total_pensiun: 0 });
      }
      ...
      row.total_pensiun = row.pens_5 + row.pens_4 + row.pens_3 + row.pens_2 + row.pens_1;
    });
  ...
  const totalsKecamatan = useMemo(() => {
    return dataKecamatan.reduce((acc, curr) => {
      acc.pens_5 += curr.pens_5;
      acc.pens_4 += curr.pens_4;
      acc.pens_3 += curr.pens_3;
      acc.pens_2 += curr.pens_2;
      acc.pens_1 += curr.pens_1;
      acc.total_pensiun += curr.total_pensiun;
      return acc;
    }, { pens_5: 0, pens_4: 0, pens_3: 0, pens_2: 0, pens_1: 0, total_pensiun: 0 });
  }, [dataKecamatan]);
  ...
  const downloadExcelRincian = async () => {
    ...
    if (activeModalTab === 'KECAMATAN') {
      const worksheet = workbook.addWorksheet('Rekap Pensiun Wilayah');
      worksheet.columns = [
        { header: isModeSemua ? 'Kabupaten/Kota' : 'Kecamatan', key: 'namaWilayah', width: 30 },
        { header: 'Usia 56 (5 Thn Lagi)', key: 'pens_5', width: 22 },
        { header: 'Usia 57 (4 Thn Lagi)', key: 'pens_4', width: 22 },
        { header: 'Usia 58 (3 Thn Lagi)', key: 'pens_3', width: 22 },
        { header: 'Usia 59 (2 Thn Lagi)', key: 'pens_2', width: 22 },
        { header: 'Usia 60 (1 Thn Lagi)', key: 'pens_1', width: 22 },
        { header: 'Total Proyeksi Pensiun', key: 'total_pensiun', width: 25 },
      ];
      dataKecamatan.forEach(item => worksheet.addRow(item));
      const totalRow = worksheet.addRow({ namaWilayah: 'TOTAL', pens_5: totalsKecamatan.pens_5, pens_4: totalsKecamatan.pens_4, pens_3: totalsKecamatan.pens_3, pens_2: totalsKecamatan.pens_2, pens_1: totalsKecamatan.pens_1, total_pensiun: totalsKecamatan.total_pensiun });
      ...
      link.download = `Rincian_Guru_Pensiun_${initialWilayah}_${safeName}.xlsx`;
      link.click();
    } else {
      const worksheet = workbook.addWorksheet('Daftar Sekolah & Pensiun');
      worksheet.columns = [
        ...
        { header: 'Usia 56', key: 'pens_5', width: 12 },
        { header: 'Usia 57', key: 'pens_4', width: 12 },
        { header: 'Usia 58', key: 'pens_3', width: 12 },
        { header: 'Usia 59', key: 'pens_2', width: 12 },
        { header: 'Usia 60', key: 'pens_1', width: 12 },
        { header: 'Total Proyeksi Pensiun', key: 'total_pensiun', width: 25 },
      ];
      ...
      const link = document.createElement('a');
      link.href = URL.createObjectURL(blob);
      link.download = `Daftar_Guru_Sekolah_Pensiun_${initialWilayah}_${safeName}.xlsx`;
      link.click();
    }
  };
  ...
  return createPortal(
    <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-in fade-in duration-200" onClick={onClose}>
      <div className="bg-white w-full max-w-7xl h-[90vh] rounded-3xl shadow-2xl flex flex-col overflow-hidden animate-in zoom-in-95 duration-300" onClick={e => e.stopPropagation()}>
        ...
        {/* HEADER MODAL */}
        <div className="bg-slate-600 px-6 py-5 flex items-center justify-between shrink-0">
          <div className="flex items-center gap-3 text-white">
            <div className="bg-white/20 p-2 rounded-xl"><CalendarDays size={24} /></div>
            <div>
              <h2 className="text-xl font-black uppercase tracking-tighter leading-none">
                Rincian Proyeksi Pensiun Guru
              </h2>
              <p className="text-slate-200 text-sm font-bold uppercase tracking-widest mt-1 flex gap-2">
          ...
          <div className="flex items-center gap-3">
            ...
            <button onClick={onClose} className="p-2 bg-slate-800 hover:bg-red-500 text-white rounded-xl transition-colors">
              <X size={24} />
            </button>
          </div>
        </div>
        ...
        <div className="bg-slate-50 px-6 pt-3 flex gap-2 border-b border-slate-100 shrink-0">
          <button 
            onClick={() => setActiveModalTab('KECAMATAN')}
            className={`px-6 py-2.5 rounded-t-xl font-black uppercase text-xs transition-all border-b-4 ${activeModalTab === 'KECAMATAN' ? 'bg-white text-slate-700 border-slate-700 shadow-[0_-4px_10px_rgba(0,0,0,0.05)]' : 'bg-transparent text-slate-500 border-transparent hover:text-slate-700 hover:bg-slate-100/50'}`}
          >
            <div className="flex items-center gap-2"><MapPin size={16}/> Per Kecamatan</div>
          </button>
          <button 
            onClick={() => setActiveModalTab('SEKOLAH')}
            className={`px-6 py-2.5 rounded-t-xl font-black uppercase text-xs transition-all border-b-4 ${activeModalTab === 'SEKOLAH' ? 'bg-white text-slate-700 border-slate-700 shadow-[0_-4px_10px_rgba(0,0,0,0.05)]' : 'bg-transparent text-slate-500 border-transparent hover:text-slate-700 hover:bg-slate-100/50'}`}
          >
        ...
        {activeModalTab === 'SEKOLAH' && (
          <div className="bg-white px-6 pt-4 pb-0 flex gap-2 overflow-x-auto scrollbar-hide shrink-0 z-10 relative">
            <button 
              ...
              className={`px-5 py-2 rounded-xl font-black uppercase text-[10px] md:text-xs transition-all whitespace-nowrap border ${activeJenjangTab === mappedJenjang ? 'bg-slate-600 text-white border-slate-600 shadow-md' : 'bg-gray-50 text-gray-500 border-gray-200 hover:bg-gray-100'}`}
            >
            ...
            {availableTabs.map(j => (
              <button 
                ...
                className={`px-5 py-2 rounded-xl font-black uppercase text-[10px] md:text-xs transition-all whitespace-nowrap border ${activeJenjangTab === j ? 'bg-slate-600 text-white border-slate-600 shadow-md' : 'bg-gray-50 text-gray-500 border-gray-200 hover:bg-gray-100'}`}
              >
                {j}
              </button>
            ))}
        ...
        <div className={`bg-white px-6 py-4 border-b border-gray-200 flex flex-wrap gap-4 items-center shrink-0 shadow-sm z-10 ${activeModalTab === 'SEKOLAH' ? 'border-t-0 pt-3' : ''}`}>
          <div className="relative flex-1 min-w-[250px]">
            ...
            <input 
              ...
              className="w-full pl-10 pr-10 py-2.5 rounded-xl border border-gray-200 outline-none focus:border-slate-500 focus:ring-2 focus:ring-slate-200 font-bold text-gray-700"
            />
          </div>
            ...
            <table className="w-full text-center border-separate border-spacing-y-2">
              <thead className="sticky top-0 bg-white z-10 shadow-sm rounded-xl">
                <tr className="text-[10px] font-black uppercase text-gray-500">
                  ...
                  <th className="px-3 py-3 text-emerald-600">Usia 56</th>
                  <th className="px-3 py-3 text-blue-600">Usia 57</th>
                  <th className="px-3 py-3 text-amber-600">Usia 58</th>
                  <th className="px-3 py-3 text-orange-600">Usia 59</th>
                  <th className="px-3 py-3 text-red-600">Usia 60</th>
                  <th className="px-4 py-3 rounded-r-xl text-slate-800">Total Proyeksi</th>
                </tr>
              </thead>
              <tbody>
                {currentRows.map((row, idx) => (
                  <tr key={idx} className="bg-white shadow-sm hover:shadow-md hover:scale-[1.01] transition-all group">
                    ...
                    <td className="px-3 py-4 font-black text-emerald-600 text-base border-y border-gray-100 bg-emerald-50/30">{row.pens_5.toLocaleString()}</td>
                    <td className="px-3 py-4 font-black text-blue-600 text-base border-y border-gray-100 bg-blue-50/30">{row.pens_4.toLocaleString()}</td>
                    <td className="px-3 py-4 font-black text-amber-600 text-base border-y border-gray-100 bg-amber-50/30">{row.pens_3.toLocaleString()}</td>
                    <td className="px-3 py-4 font-black text-orange-600 text-base border-y border-gray-100 bg-orange-50/30">{row.pens_2.toLocaleString()}</td>
                    <td className="px-3 py-4 font-black text-red-600 text-base border-y border-gray-100 bg-red-50/20">{row.pens_1.toLocaleString()}</td>

                    <td className="px-4 py-4 font-black text-slate-800 text-lg border-y border-r border-gray-100 bg-slate-100/80 rounded-r-2xl">{row.total_pensiun.toLocaleString()}</td>
                  </tr>
                ))}
              </tbody>
              ...
              {dataKecamatan.length > 0 && (
                <tfoot className="sticky bottom-0 z-10 shadow-[0_-4px_10px_rgba(0,0,0,0.04)]">
                  <tr className="bg-slate-100 text-center font-black uppercase text-xs border-t-2 border-slate-200">
                    <td colSpan="2" className="px-4 py-4 text-left rounded-l-2xl border-y border-l border-slate-200 text-slate-900">
                      TOTAL {isModeSemua ? 'KESELURUHAN' : 'KECAMATAN'}
                    </td>
                    <td className="px-3 py-4 text-emerald-800 text-base border-y border-slate-200">{totalsKecamatan.pens_5.toLocaleString()}</td>
                    <td className="px-3 py-4 text-blue-800 text-base border-y border-slate-200">{totalsKecamatan.pens_4.toLocaleString()}</td>
                    <td className="px-3 py-4 text-amber-800 text-base border-y border-slate-200">{totalsKecamatan.pens_3.toLocaleString()}</td>
                    <td className="px-3 py-4 text-orange-800 text-base border-y border-slate-200">{totalsKecamatan.pens_2.toLocaleString()}</td>
                    <td className="px-3 py-4 text-red-800 text-base border-y border-slate-200">{totalsKecamatan.pens_1.toLocaleString()}</td>
                    <td className="px-4 py-4 text-slate-950 text-lg border-y border-r border-slate-200 rounded-r-2xl bg-slate-200/50">
                      {totalsKecamatan.total_pensiun.toLocaleString()}
                    </td>
                  </tr>
                </tfoot>
              )}
            ...
            <table className="w-full text-center border-separate border-spacing-y-2">
              <thead className="sticky top-0 bg-white z-10 shadow-sm rounded-xl">
                <tr className="text-[10px] font-black uppercase text-gray-500">
                  ...
                  <th className="px-3 py-3 text-slate-600">Jenjang</th>
              ...
              <tbody>
                {currentRows.map((row, idx) => (
                  <tr key={idx} className="bg-white shadow-sm hover:shadow-md hover:scale-[1.01] transition-all group">
                    ...
                    <td className="px-3 py-3 font-bold text-slate-600 text-xs border-y border-gray-100 uppercase">{row.jenjang}</td>
                    <td className="px-3 py-3 font-bold text-orange-600 text-xs border-y border-gray-100 uppercase">{row.status}</td>
                    
                    <td className="px-3 py-3 font-bold text-emerald-600 text-sm border-y border-gray-100 bg-emerald-50/20">{row.pens_5.toLocaleString()}</td>
                    <td className="px-3 py-3 font-bold text-blue-600 text-sm border-y border-gray-100 bg-blue-50/20">{row.pens_4.toLocaleString()}</td>
                    <td className="px-3 py-3 font-bold text-amber-600 text-sm border-y border-gray-100 bg-amber-50/20">{row.pens_3.toLocaleString()}</td>
                    <td className="px-3 py-3 font-bold text-orange-600 text-sm border-y border-gray-100 bg-orange-50/20">{row.pens_2.toLocaleString()}</td>
                    <td className="px-3 py-3 font-bold text-red-600 text-sm border-y border-gray-100 bg-red-50/20">{row.pens_1.toLocaleString()}</td>

                    <td className="px-4 py-3 font-black text-slate-800 text-lg border-y border-r border-gray-100 bg-slate-100/80 rounded-r-2xl">
                      {row.total_pensiun.toLocaleString()}
                    </td>
                  </tr>
                ))}
              </tbody>
              ...
              {dataSekolah.length > 0 && (
                <tfoot className="sticky bottom-0 z-10 shadow-[0_-4px_10px_rgba(0,0,0,0.04)]">
                  <tr className="bg-slate-100 text-center font-black uppercase text-xs border-t-2 border-slate-200">
                    <td colSpan="6" className="px-4 py-4 text-left rounded-l-2xl border-y border-l border-slate-200 text-slate-900">
                      TOTAL PROYEKSI PENSIUN DARI {dataSekolah.length} SEKOLAH
                    </td>
                    <td className="px-3 py-4 text-emerald-800 text-base border-y border-slate-200">{totalGuruSekolah.pens_5.toLocaleString()}</td>
                    <td className="px-3 py-4 text-blue-800 text-base border-y border-slate-200">{totalGuruSekolah.pens_4.toLocaleString()}</td>
                    <td className="px-3 py-4 text-amber-800 text-base border-y border-slate-200">{totalGuruSekolah.pens_3.toLocaleString()}</td>
                    <td className="px-3 py-4 text-orange-800 text-base border-y border-slate-200">{totalGuruSekolah.pens_2.toLocaleString()}</td>
                    <td className="px-3 py-4 text-red-800 text-base border-y border-slate-200">{totalGuruSekolah.pens_1.toLocaleString()}</td>
                    <td className="px-4 py-4 text-slate-950 text-lg border-y border-r border-slate-200 rounded-r-2xl bg-slate-200/50">
                      {totalGuruSekolah.total_pensiun.toLocaleString()}
                    </td>
                  </tr>
                </tfoot>
              )}
        ...
        <div className="bg-white p-4 border-t border-gray-200 flex items-center justify-between shrink-0 rounded-b-3xl">
          <div className="flex flex-col">
            <p className="text-xs font-bold text-gray-500">
              Menampilkan <span className="text-gray-800">{activeData.length === 0 ? 0 : startIndex + 1}</span> - <span className="text-gray-800">{Math.min(startIndex + rowsPerPage, activeData.length)}</span> dari <span className="text-slate-700 font-black">{activeData.length}</span> baris
            </p>
            {displayLastUpdated && (
          ...
          <div className="flex items-center gap-2">
            <button disabled={currentPage === 1} onClick={() => goToPage(currentPage - 1)} className="p-2 rounded-xl bg-gray-50 border border-gray-200 hover:bg-slate-50 disabled:opacity-50 transition-colors"><ChevronLeft size={16} /></button>
            <span className="text-xs font-black text-gray-600 px-2">Hal {currentPage} / {totalPages}</span>
            <button disabled={currentPage === totalPages || totalPages === 0} onClick={() => goToPage(currentPage + 1)} className="p-2 rounded-xl bg-gray-50 border border-gray-200 hover:bg-slate-50 disabled:opacity-50 transition-colors"><ChevronRight size={16} /></button>
          </div>
        </div>
