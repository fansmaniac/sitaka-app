...
import { 
  Download, CalendarDays, MapPin, Search, X, ChevronLeft, ChevronRight, Building2 
} from 'lucide-react';
...
export default function RincianProyeksiPensiunGuru({ 
  ...
  // Proses Agregasi Data Proyeksi Pensiun
  const processedData = useMemo(() => {
    ...
    validData.forEach(item => {
      ...
      if (age !== null) {
          if (age === 56) { row.pens_5++; row.total_pensiun++; }
          else if (age === 57) { row.pens_4++; row.total_pensiun++; }
          else if (age === 58) { row.pens_3++; row.total_pensiun++; }
          else if (age === 59) { row.pens_2++; row.total_pensiun++; }
          else if (age === 60) { row.pens_1++; row.total_pensiun++; }
      }
    });
  ...
  const downloadExcelRincian = async () => {
    ...
    worksheet.columns = [
      ...
      { header: 'Usia 56 (5 Thn)', key: 'pens_5', width: 15 },
      { header: 'Usia 57 (4 Thn)', key: 'pens_4', width: 15 },
      { header: 'Usia 58 (3 Thn)', key: 'pens_3', width: 15 },
      { header: 'Usia 59 (2 Thn)', key: 'pens_2', width: 15 },
      { header: 'Usia 60 (1 Thn)', key: 'pens_1', width: 15 },
    ...
    const totalRow = worksheet.addRow({
      ...
      pens_5: columnTotals.pens_5,
      pens_4: columnTotals.pens_4,
      pens_3: columnTotals.pens_3,
      pens_2: columnTotals.pens_2,
      pens_1: columnTotals.pens_1,
      total_pensiun: columnTotals.total_pensiun
    });

    // Styling Header
    worksheet.getRow(1).font = { bold: true, color: { argb: 'FFFFFFFF' } };
    worksheet.getRow(1).fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FF334155' } }; // Slate 700
    
    // Styling Baris Total
    totalRow.font = { bold: true, color: { argb: 'FF0F172A' } }; // Slate 900
    ...
    link.download = `Rincian_Proyeksi_Pensiun_${isModeSemua ? 'Provinsi' : filterWilayah}_${activeJenjang.replace('/','-')}_${filterStatus}.xlsx`;
    link.click();
  };
  ...
  return createPortal(
    <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-in fade-in duration-200" onClick={onClose}>
      <div className="bg-white w-full max-w-6xl h-[90vh] rounded-3xl shadow-2xl flex flex-col overflow-hidden animate-in zoom-in-95 duration-300" onClick={e => e.stopPropagation()}>
        ...
        {/* HEADER MODAL */}
        <div className="bg-slate-700 px-6 py-5 flex items-center justify-between shrink-0">
          <div className="flex items-center gap-3 text-white">
            ...
            <div>
              ...
              <p className="text-slate-300 text-sm font-bold uppercase tracking-widest mt-1 flex gap-2">
          ...
          <table className="w-full text-center border-separate border-spacing-y-2">
            <thead className="sticky top-0 bg-white z-10 shadow-sm rounded-xl">
              <tr className="text-[10px] font-black uppercase text-gray-500">
                ...
                <th className="px-3 py-3 text-emerald-600">Usia 56 (5 Thn)</th>
                <th className="px-3 py-3 text-blue-600">Usia 57 (4 Thn)</th>
                <th className="px-3 py-3 text-amber-600">Usia 58 (3 Thn)</th>
                <th className="px-3 py-3 text-orange-600">Usia 59 (2 Thn)</th>
                <th className="px-3 py-3 text-red-600">Usia 60 (1 Thn)</th>
              ...
              {currentRows.map((row, idx) => (
                <tr key={idx} className="bg-white shadow-sm hover:shadow-md hover:scale-[1.01] transition-all group">
                  ...
                  <td className="px-3 py-4 font-black text-emerald-600 text-base border-y border-gray-100 bg-emerald-50/30">
                    {row.pens_5.toLocaleString()}
                  </td>
                  <td className="px-3 py-4 font-black text-blue-600 text-base border-y border-gray-100 bg-blue-50/30">
                    {row.pens_4.toLocaleString()}
                  </td>
                  <td className="px-3 py-4 font-black text-amber-600 text-base border-y border-gray-100 bg-amber-50/30">
                    {row.pens_3.toLocaleString()}
                  </td>
                  <td className="px-3 py-4 font-black text-orange-600 text-base border-y border-gray-100 bg-orange-50/30">
                    {row.pens_2.toLocaleString()}
                  </td>
                  <td className="px-3 py-4 font-black text-red-600 text-base border-y border-gray-100 bg-red-50/30">
                    {row.pens_1.toLocaleString()}
                  </td>
                  <td className="px-4 py-4 font-black text-slate-800 text-lg border-y border-r border-gray-100 bg-slate-50/50 rounded-r-2xl">
            ...
            {processedData.length > 0 && (
              <tfoot className="sticky bottom-0 z-10 shadow-[0_-4px_10px_rgba(0,0,0,0.04)]">
                <tr className="bg-slate-100 text-center font-black uppercase text-xs border-t-2 border-slate-200">
                  ...
                  <td className="px-3 py-4 text-emerald-800 text-base border-y border-slate-200">{columnTotals.pens_5.toLocaleString()}</td>
                  <td className="px-3 py-4 text-blue-800 text-base border-y border-slate-200">{columnTotals.pens_4.toLocaleString()}</td>
                  <td className="px-3 py-4 text-amber-800 text-base border-y border-slate-200">{columnTotals.pens_3.toLocaleString()}</td>
                  <td className="px-3 py-4 text-orange-800 text-base border-y border-slate-200">{columnTotals.pens_2.toLocaleString()}</td>
                  <td className="px-3 py-4 text-red-800 text-base border-y border-slate-200">{columnTotals.pens_1.toLocaleString()}</td>
                  <td className="px-4 py-4 text-slate-950 text-lg border-y border-r border-slate-200 rounded-r-2xl bg-slate-200/50">
                    {columnTotals.total_pensiun.toLocaleString()}
                  </td>
                </tr>
              </tfoot>
            )}
        ...
        <div className="bg-gray-50 p-4 border-t border-gray-200 flex items-center justify-between shrink-0 rounded-b-3xl">
          <div className="flex flex-col">
            <p className="text-xs font-bold text-gray-500">
              Menampilkan <span className="text-gray-800">{processedData.length === 0 ? 0 : startIndex + 1}</span> - <span className="text-gray-800">{Math.min(startIndex + rowsPerPage, processedData.length)}</span> dari <span className="text-slate-700 font-black">{processedData.length}</span> baris
            </p>
            {displayLastUpdated && (
          ...
          <div className="flex items-center gap-2">
            <button disabled={currentPage === 1} onClick={() => goToPage(currentPage - 1)} className="p-2 rounded-xl bg-white border border-gray-200 hover:bg-slate-100 disabled:opacity-50 transition-colors"><ChevronLeft size={16} /></button>
            <span className="text-xs font-black text-gray-600 px-2">Hal {currentPage} / {totalPages}</span>
            <button disabled={currentPage === totalPages || totalPages === 0} onClick={() => goToPage(currentPage + 1)} className="p-2 rounded-xl bg-white border border-gray-200 hover:bg-slate-100 disabled:opacity-50 transition-colors"><ChevronRight size={16} /></button>
          </div>
        </div>
