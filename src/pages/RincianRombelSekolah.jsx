...
import { 
  Download, Layers, MapPin, Search, X, ChevronLeft, ChevronRight, 
  Building2, School
} from 'lucide-react';
  ...
  const downloadExcelRincian = async () => {
    ...
    if (activeModalTab === 'KECAMATAN') {
      ...
      worksheet.columns = [
        ...
        { header: 'Rombel Negeri', key: 'negeri', width: 18 },
        { header: 'Rombel Swasta', key: 'swasta', width: 18 },
        { header: 'Total Rombel', key: 'total', width: 18 },
      ];
      ...
      worksheet.columns = [
        ...
        { header: 'Jumlah Rombel', key: 'rombel', width: 18 },
      ];
      ...
      <div className="bg-white w-full max-w-6xl h-[90vh] rounded-3xl shadow-2xl flex flex-col overflow-hidden animate-in zoom-in-95 duration-300" onClick={e => e.stopPropagation()}>
        ...
        <div className="bg-indigo-50 px-6 pt-3 flex gap-2 border-b border-indigo-100 shrink-0">
          <button 
            ...
            className={`px-6 py-2.5 rounded-t-xl font-black uppercase text-xs transition-all border-b-4 ${activeModalTab === 'KECAMATAN' ? 'bg-white text-indigo-700 border-indigo-700 shadow-[0_-4px_10px_rgba(0,0,0,0.05)]' : 'bg-transparent text-indigo-400 border-transparent hover:text-indigo-600 hover:bg-indigo-100/50'}`}
          >
            <div className="flex items-center gap-2"><MapPin size={16}/> Per Kecamatan</div>
          </button>
          <button 
            onClick={() => setActiveModalTab('SEKOLAH')}
            className={`px-6 py-2.5 rounded-t-xl font-black uppercase text-xs transition-all border-b-4 ${activeModalTab === 'SEKOLAH' ? 'bg-white text-indigo-700 border-indigo-700 shadow-[0_-4px_10px_rgba(0,0,0,0.05)]' : 'bg-transparent text-indigo-400 border-transparent hover:text-indigo-600 hover:bg-indigo-100/50'}`}
          >
            ...
            <table className="w-full text-center border-separate border-spacing-y-2">
              <thead className="sticky top-0 bg-white z-10 shadow-sm rounded-xl">
                <tr className="text-[10px] font-black uppercase text-gray-500">
                  ...
                  <th className="px-4 py-3 text-indigo-600">Rombel Negeri</th>
              ...
              <tbody>
                {currentRows.map((row, idx) => (
                  <tr key={idx} className="bg-white shadow-sm hover:shadow-md hover:scale-[1.01] transition-all group">
                    ...
                    <td className="px-4 py-4 font-black text-indigo-600 text-base border-y border-gray-100 bg-indigo-50/30">
              ...
              {dataKecamatan.length > 0 && (
                <tfoot className="sticky bottom-0 z-10 shadow-[0_-4px_10px_rgba(0,0,0,0.04)]">
                  <tr className="bg-indigo-100 text-center font-black uppercase text-xs border-t-2 border-indigo-200">
                    ...
                    <td className="px-4 py-4 text-indigo-800 text-base border-y border-indigo-200">{totalsKecamatan.negeri.toLocaleString()}</td>
                    <td className="px-4 py-4 text-orange-800 text-base border-y border-indigo-200">{totalsKecamatan.swasta.toLocaleString()}</td>
            ...
            <table className="w-full text-center border-separate border-spacing-y-2">
              <thead className="sticky top-0 bg-white z-10 shadow-sm rounded-xl">
                <tr className="text-[10px] font-black uppercase text-gray-500">
                  ...
                  <th className="px-3 py-3 text-indigo-600">Jenjang</th>
                  <th className="px-3 py-3 text-orange-600">Status</th>
                  <th className="px-4 py-3 rounded-r-xl text-gray-800">Jumlah Rombel</th>
                </tr>
              </thead>
              <tbody>
                {currentRows.map((row, idx) => (
                  <tr key={idx} className="bg-white shadow-sm hover:shadow-md hover:scale-[1.01] transition-all group">
                    ...
                    <td className="px-3 py-3 font-bold text-indigo-600 text-xs border-y border-gray-100 uppercase">{row.jenjang}</td>
              ...
              {dataSekolah.length > 0 && (
                <tfoot className="sticky bottom-0 z-10 shadow-[0_-4px_10px_rgba(0,0,0,0.04)]">
                  <tr className="bg-indigo-100 text-center font-black uppercase text-xs border-t-2 border-indigo-200">
                    <td colSpan="6" className="px-4 py-4 text-left rounded-l-2xl border-y border-l border-indigo-200 text-indigo-900">
                      TOTAL KESELURUHAN ROMBEL DARI {dataSekolah.length} SEKOLAH
                    </td>
                    <td className="px-4 py-4 text-indigo-950 text-lg border-y border-r border-indigo-200 rounded-r-2xl bg-indigo-200/50">
                      {totalRombelSekolah.toLocaleString()}
                    </td>
                  </tr>
                </tfoot>
              )}
