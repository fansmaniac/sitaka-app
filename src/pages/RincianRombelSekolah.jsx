...
import { 
  Download, Layers, MapPin, Search, X, ChevronLeft, ChevronRight, Building2 
} from 'lucide-react';
...
// MAIN COMPONENT
// =====================================================================
export default function RincianRombelSekolah({ 
  isOpen, 
  onClose, 
  data = [], 
  ...
  // Sinkronisasi saat modal dibuka dengan kabupaten yang diklik
  useEffect(() => {
    if (isOpen) setFilterWilayah(initialWilayah);
  }, [isOpen, initialWilayah]);

  // Reset halaman ke 1 jika ada pencarian atau ganti filter
  useEffect(() => { 
    setCurrentPage(1); 
  ...
  const processedData = useMemo(() => {
    ...
    validData.forEach(item => {
      ...
      if (!mapAgg.has(keyId)) {
        mapAgg.set(keyId, { namaWilayah: keyId, rombel_n: 0, rombel_s: 0, total: 0 });
      }

      const row = mapAgg.get(keyId);
      const isNegeri = String(getVal(item, 'status_sekolah')).toUpperCase() === 'NEGERI';

      // Logika dinamis penarikan jumlah rombel yang tahan banting (termasuk yg typo ada spasi)
      let rombelTotal = 0;
      ...
      if (isNegeri) row.rombel_n += rombelTotal;
      else row.rombel_s += rombelTotal;
      
      row.total += rombelTotal;
    });
  ...
  // Hitung Grand Total Baris Bawah
  ...
  const downloadExcelRincian = async () => {
    ...
    worksheet.columns = [
      ...
      { header: 'Total Rombongan Belajar', key: 'total', width: 25 },
    ];
    ...
    const totalRow = worksheet.addRow({
      ...
      rombel_n: columnTotals.rombel_n,
      rombel_s: columnTotals.rombel_s,
      total: columnTotals.total
    });

    // Style Header
    worksheet.getRow(1).font = { bold: true, color: { argb: 'FFFFFFFF' } };
    worksheet.getRow(1).fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FFC2410C' } }; // Orange 700
    ...
    link.download = `Rincian_Rombel_${isModeSemua ? 'Provinsi' : filterWilayah}_${activeJenjang.replace('/','-')}_${filterStatus}.xlsx`;
    link.click();
  };
  ...
  return createPortal(
    <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-in fade-in duration-200" onClick={onClose}>
      <div className="bg-white w-full max-w-5xl h-[90vh] rounded-3xl shadow-2xl flex flex-col overflow-hidden animate-in zoom-in-95 duration-300" onClick={e => e.stopPropagation()}>
        ...
        {/* HEADER MODAL */}
        <div className="bg-orange-700 px-6 py-5 flex items-center justify-between shrink-0">
          <div className="flex items-center gap-3 text-white">
            <div className="bg-white/20 p-2 rounded-xl"><Layers size={24} /></div>
            <div>
              <h2 className="text-xl font-black uppercase tracking-tighter leading-none">
                Rincian Rombongan Belajar
              </h2>
              <p className="text-orange-200 text-sm font-bold uppercase tracking-widest mt-1 flex gap-2">
                <span>{isModeSemua ? 'Provinsi Kalimantan Barat' : `Kecamatan di Kab. ${filterWilayah}`}</span>
                <span className="opacity-50">|</span>
                <span className="text-amber-100">Jenjang: {activeJenjang}</span>
              </p>
            </div>
          </div>
          <div className="flex items-center gap-3">
            ...
            <button onClick={onClose} className="p-2 bg-orange-800 hover:bg-red-500 text-white rounded-xl transition-colors">
              <X size={24} />
            </button>
          </div>
        </div>
        ...
        <div className="bg-gray-50 px-6 py-4 border-b border-gray-200 flex flex-wrap gap-4 items-center shrink-0">
          <div className="relative flex-1 min-w-[250px]">
            ...
            <input 
              ...
              className="w-full pl-10 pr-10 py-2.5 rounded-xl border border-gray-200 outline-none focus:border-orange-500 focus:ring-2 focus:ring-orange-200 font-bold text-gray-700"
            />
          </div>
          ...
          <table className="w-full text-center border-separate border-spacing-y-2">
            <thead className="sticky top-0 bg-white z-10 shadow-sm rounded-xl">
              <tr className="text-[10px] font-black uppercase text-gray-500">
                ...
                <th className="px-4 py-3 text-blue-600">Rombel Negeri</th>
                <th className="px-4 py-3 text-orange-600">Rombel Swasta</th>
                <th className="px-4 py-3 rounded-r-xl text-gray-800">Total Rombel</th>
              </tr>
            </thead>
            <tbody>
            ...
            {processedData.length > 0 && (
              <tfoot className="sticky bottom-0 z-10 shadow-[0_-4px_10px_rgba(0,0,0,0.04)]">
                <tr className="bg-orange-100 text-center font-black uppercase text-xs border-t-2 border-orange-200">
                  ...
                  <td className="px-4 py-4 text-blue-800 text-base border-y border-orange-200">{columnTotals.rombel_n.toLocaleString()}</td>
                  <td className="px-4 py-4 text-orange-800 text-base border-y border-orange-200">{columnTotals.rombel_s.toLocaleString()}</td>
                  <td className="px-4 py-4 text-orange-950 text-lg border-y border-r border-orange-200 rounded-r-2xl bg-orange-200/50">
        ...
        <div className="bg-gray-50 p-4 border-t border-gray-200 flex items-center justify-between shrink-0 rounded-b-3xl">
          <div className="flex flex-col">
            <p className="text-xs font-bold text-gray-500">
              Menampilkan <span className="text-gray-800">{processedData.length === 0 ? 0 : startIndex + 1}</span> - <span className="text-gray-800">{Math.min(startIndex + rowsPerPage, processedData.length)}</span> dari <span className="text-orange-700 font-black">{processedData.length}</span> baris
            </p>
            {displayLastUpdated && (
          ...
          <div className="flex items-center gap-2">
            <button disabled={currentPage === 1} onClick={() => goToPage(currentPage - 1)} className="p-2 rounded-xl bg-white border border-gray-200 hover:bg-orange-50 disabled:opacity-50 transition-colors"><ChevronLeft size={16} /></button>
            <span className="text-xs font-black text-gray-600 px-2">Hal {currentPage} / {totalPages}</span>
            <button disabled={currentPage === totalPages || totalPages === 0} onClick={() => goToPage(currentPage + 1)} className="p-2 rounded-xl bg-white border border-gray-200 hover:bg-orange-50 disabled:opacity-50 transition-colors"><ChevronRight size={16} /></button>
          </div>
        </div>
