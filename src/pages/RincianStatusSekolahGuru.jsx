...
export default function RincianStatusSekolahGuru({ 
  ...
  const dataSekolah = useMemo(() => {
    ...
    validData.forEach(item => {
      ...
      // Setiap item data PTK yang masuk = 1 Guru
      mapSekolah.get(keyId).jumlah_guru++;
    });
  ...
  const totalGuruSekolah = useMemo(() => {
    return dataSekolah.reduce((acc, curr) => acc + curr.jumlah_guru, 0);
  }, [dataSekolah]);
  ...
  const downloadExcelRincian = async () => {
    ...
    if (activeModalTab === 'KECAMATAN') {
      ...
      worksheet.columns = [
        ...
        { header: 'Guru Sekolah Negeri', key: 'negeri', width: 20 },
        { header: 'Guru Sekolah Swasta', key: 'swasta', width: 20 },
      ...
      link.download = `Rincian_Guru_Status_Kecamatan_${initialWilayah}.xlsx`;
      link.click();
    } else {
      // Tab SEKOLAH
      ...
      worksheet.columns = [
        ...
        { header: 'Jumlah Guru', key: 'jumlah_guru', width: 18 },
      ];
      ...
      const link = document.createElement('a');
      link.href = URL.createObjectURL(blob);
      link.download = `Daftar_Sekolah_Guru_Status_${initialWilayah}_${activeJenjangTab.replace(' ','_')}.xlsx`;
      link.click();
    }
  };
        ...
        {activeModalTab === 'SEKOLAH' && (
          <div className="bg-white px-6 pt-4 pb-0 flex gap-2 overflow-x-auto scrollbar-hide shrink-0 z-10 relative">
            <button 
              ...
              className={`px-5 py-2 rounded-xl font-black uppercase text-[10px] md:text-xs transition-all whitespace-nowrap border ${activeJenjangTab === 'SEMUA' ? 'bg-blue-600 text-white border-blue-600 shadow-md' : 'bg-gray-50 text-gray-500 border-gray-200 hover:bg-gray-100'}`}
            >
              Semua Jenjang
            </button>
            ...
            <table className="w-full text-center border-separate border-spacing-y-2">
              <thead className="sticky top-0 bg-white z-10 shadow-sm rounded-xl">
                <tr className="text-[10px] font-black uppercase text-gray-500">
                  ...
                  <th className="px-4 py-3 rounded-r-xl text-gray-800">Jumlah Guru</th>
                </tr>
              </thead>
              <tbody>
                {currentRows.map((row, idx) => (
                  <tr key={idx} className="bg-white shadow-sm hover:shadow-md hover:scale-[1.01] transition-all group">
                    ...
                    <td className="px-4 py-3 font-black text-gray-800 text-lg border-y border-r border-gray-100 bg-gray-50/50 rounded-r-2xl">
                      {row.jumlah_guru.toLocaleString()}
                    </td>
                  </tr>
                ))}
              </tbody>
              ...
              {dataSekolah.length > 0 && (
                <tfoot className="sticky bottom-0 z-10 shadow-[0_-4px_10px_rgba(0,0,0,0.04)]">
                  <tr className="bg-blue-100 text-center font-black uppercase text-xs border-t-2 border-blue-200">
                    ...
                    <td className="px-4 py-4 text-blue-950 text-lg border-y border-r border-blue-200 rounded-r-2xl bg-blue-200/50">
                      {totalGuruSekolah.toLocaleString()}
                    </td>
                  </tr>
                </tfoot>
              )}
